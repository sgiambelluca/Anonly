...
- **ADR-067 no toca este motor.** El orden de lectura de `pdf-engine` pasó a agrupar en runs los words con `bbox.rotation` 90/180/270, pero este motor **nunca puebla `rotation`**: Tesseract no reporta orientación por palabra y el kernel no la infiere, así que todo word de OCR entra por la rama horizontal, que es literalmente la de siempre. Si algún día este motor aprendiera a reconocer texto rotado, poblar `rotation` alcanzaría para que el orden de `pdf-engine` lo cubra — sería un ADR propio, no una reconciliación de dos criterios.
