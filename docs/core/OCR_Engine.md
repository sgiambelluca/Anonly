...
> **Nota (v1.8.0, ADR-121, 2026-09-02 — el sello rotado vive en el margen)**: el hueco que §13 caso 15 dejó anotado se cierra. El buscador de líneas de Tesseract busca líneas de base **horizontales**, así que una corrida vertical dentro de una página derecha no es una línea y no la encuentra: medido sobre `qa-stamp.pdf`, de sus **15 palabras rotadas la pasada derecha recupera 2**, con `AUTO` y con `SPARSE_TEXT` por igual. Y OSD no ayuda, aunque suene a que debería — contesta **bien**, porque la orientación dominante de esa página *es* 0°; enderezarla para leer el sello pondría el cuerpo de costado. Es un problema distinto del de ADR-119. Ahora, después de la pasada derecha, el kernel recorta una **franja de margen de cada lado** (`MARGIN_STRIP_RATIO = 0,2` del ancho) y reconoce cada una a **90° y 270°** con la maquinaria de ADR-090 §3 (`rotateImageData`/`unrotateBbox`). De la unión se descarta lo que **solapa** una palabra ya leída derecha —el umbral no hay que elegirlo: 0,01, 0,3 y 0,5 dan los mismos 18 sobrantes— y lo que cae por debajo de `ROTATED_MIN_CONFIDENCE = 60`. Resultado: **15/15 palabras rotadas y 70/73 del total**, contra 55/73 antes, con 6-7 sobrantes; y sobre `text-10p.pdf`, **que no tiene texto rotado, las cuatro pasadas aportan 0 palabras**. Ese último número es el que permite dejarlo **siempre encendido**, sin campo de `OcrConfig` y sin preferencia de usuario. Costo: **+70 % de tiempo de OCR** por página escaneada (2716 ms contra 1600 ms); las tres pasadas de página completa recuperaban lo mismo por +133 %, y traían 33 palabras inventadas del cuerpo leído de costado que el recorte directamente no deja entrar. Un fallo de una franja **no voltea la página**; una cancelación sí se propaga. **Lo que no cubre**: un sello rotado en el **medio** de la página — es una apuesta sobre dónde vive el dato, no una garantía. Ver §10, §13 caso 16, §14 y §15 item 26.

> **Nota (v1.7.0, ADR-119, 2026-09-02 — la orientación se detecta con el motor que la sabe leer)**: la detección de orientación de ADR-090 **nunca funcionó**. `worker.detect()` necesita, además del core legacy que ADR-090 §1 dedujo, que el **OEM** sea legacy; con el default —LSTM— no tira pero devuelve `orientation_degrees: 0, orientation_confidence: 0` **siempre**, y el piso de confianza lo descarta. Medido sobre dos documentos y cuatro orientaciones cada uno. La consecuencia es total: una hoja escaneada girada 90° se lee **1 de 268 palabras**; enderezada, 266. Y el OEM legacy no puede convivir con el reconocimiento, porque el `traineddata` pineado es `tessdata_best`, sin componentes legacy para `spa`/`eng`. Ahora hay un **worker dedicado a OSD** (`["osd"]`, `OEM.TESSERACT_ONLY`, `legacyCore: true`) y el principal deja de cargar `osd` y de pedir `legacyCore`. La detección corre sobre el raster a **media escala**: acierta las cuatro orientaciones con confianza 12-16 y cuesta 290 ms por página, contra los **506 ms que hoy se pagan por una detección que no funciona** — el arreglo sale más barato que el estado actual. Por debajo de 0,35 el modo de falla es peligroso: a 0,25 acierta cero veces y sigue devolviendo confianza por encima del piso. La geometría de ADR-090 queda **verificada**: OSD devuelve el ángulo de corrección, que es la convención de `rotateImageData`, y `unrotateBbox` devuelve el 100 % de las cajas sobre tinta en las tres rotaciones. **Lo que este ADR no cierra**: con la orientación arreglada el texto se reconoce pero llega desordenado (132-152 de 267 pares consecutivos), porque `bbox.rotation` lo manda por la rama de runs rotados de ADR-067; el arreglo es de `pdf-engine` y va aparte. Ver §13 y §14.

> **Nota (v1.6.0, ADR-112, 2026-09-02 — el sello no es un párrafo)**: con `tessedit_pageseg_mode` en su default (`AUTO`), el análisis de layout de Tesseract **fusiona en una sola caja de línea** dos renglones impresos muy juntos y devuelve basura para la mitad izquierda de la fusión. Sobre el sello del encabezado de un fallo escaneado —`IPP …` justo encima de `APELLIDO, NOMBRE S/ RECURSO DE`— el apellido del imputado se lee `casera,`, `suarna,` o `sez,` según la página, y un valor mal leído no lo detecta nadie: el dato queda a la vista **sin dejar rastro en el PDF exportado**. Es también la explicación de las cajas con 60 % de diferencia de alto que ADR-110 §5 reportó sin explicar — son cajas de dos renglones. El kernel pasa a fijar `PSM.SPARSE_TEXT` (11), una vez por instancia, junto al `user_defined_dpi` de ADR-090 §2 y con el mismo criterio best-effort. Medido sobre 19 páginas contra la transcripción a mano del sello: **57/114 ítems → 109/114**, y el apellido pasa de detectarse en 9 de 19 páginas a 19 de 19. Sobre 7 documentos nativos rasterizados (35 páginas, 11.403 palabras) el recall queda igual (96,29 % → 96,26 %) y la **precisión sube** (96,02 % → 96,50 %): el modo disperso inventa menos, no más. Costo: +8 % de tiempo de OCR por página. No es un campo de `OcrConfig` y no toca ningún contrato. Ver §13 caso 15 y §14.
...
| `recovers rotated text that the upright pass cannot see` | `unit.test.ts` | unit | caso 16 — el mecanismo: 5 pasadas (1 derecha + 4 de franja) y la palabra rotada entra etiquetada con `bbox.rotation`, que es lo que la manda por la rama de ADR-067 §4 |
| `adds nothing on a page with no rotated text (ADR-121, el caso normal)` | `unit.test.ts` | unit | caso 16 — **el número que permite dejarlo siempre encendido**: sin texto rotado las cuatro pasadas aportan 0 |
| `discards a rotated candidate that overlaps a word already read upright` | `unit.test.ts` | unit | caso 16 — la mitad de la regla de fusión; sin ella, 33 sobrantes en vez de 18. Ojo con la geometría: la franja derecha mapea lejos de la izquierda, así que una candidata repetida en las cuatro pasadas deja **dos** sobrevivientes, no cero |
| `discards a rotated candidate below the confidence floor` | `unit.test.ts` | unit | caso 16 — la otra mitad: de 18 sobrantes a 6 |
| `does not fail the page when a margin pass throws` | `unit.test.ts` | unit | caso 16 — perder el texto derecho por un extra sería peor que no tener el extra |
