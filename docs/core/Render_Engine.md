...
>
> 1. **`RenderPageOutput.imageData` pasa a `imageData?: ImageData`** (§6, §10) y **queda siempre ausente en la práctica**: ningún camino de hoy lo puebla, en ninguno de los dos `mode`. El campo se conserva opcional "por si algún consumidor futuro lo necesita" — deuda declarada (ADR-156, Consecuencias): si en un año nadie lo pobló nunca, corresponde sacarlo.
> 2. **El kernel deja de transportar los píxeles crudos en `mode: "preview"`**: `KernelRenderResult.imageData` pasa a opcional (`worker/kernel.ts`) y `kernelRenderPage` solo lo incluye en su resultado cuando `mode === "full"` — ahí no cambia nada, el kernel sigue devolviendo `{ imageData, encoded, degraded }` como siempre, y la razón es exclusivamente conservar el alcance del cambio acotado a preview (ADR-156, "Lo que no toca"). El **ahorro mayor no es dejar de guardarlos, es dejar de materializarlos en el host**: antes, cada preview cruzaba el `postMessage` con una copia completa del RGBA (hasta ~32 MB por página a `MAX_RENDER_SCALE`) solo para que el host la descartara. El decoder de ADR-055 (`isKernelRenderResult`) acepta `imageData` ausente o válido — sigue siendo un guard explícito, nunca un cast.
> 3. **El cache LRU de previews guarda solo lo codificado**: `InternalCacheEntry` deja de llevar `imageData` (en ningún `mode` — ver punto 1) y `estimateEntryBytes` (§12) cuenta solo `encoded.bytes.byteLength`. A `MAX_RENDER_SCALE` eso es ~1 MB por página en vez de ~32 MB; el tope `PREVIEW_CACHE_MAX_BYTES` de 200 MB pasa a acotar del orden de diez veces más páginas de lo que acotaba antes de este cambio.
>
> **Lo que no cambia** (alcance de ADR-156; `rasterizePage` **sí** cambia, por ADR-158 — ver la nota siguiente): `rasterizePage` y el camino de OCR no participan de este cache ni de `RenderPageOutput`; `emitPreviewUpdated`, que ya construía el blob desde `entry.encoded.bytes`, nunca desde `imageData`; el veredicto de degradación de ADR-062 (sigue saliendo de la entrada, intacto); ningún evento, error code ni tipo de `Contracts.md` (`RenderPageOutput` no está replicado ahí — vive solo en este spec, §6/§10; nada que actualizar en `Contracts.md` §5 para este cambio). Ver §6, §10, §12, §13.

> **Nota (v1.17.0, ADR-158, 2026-09-12 — el ráster de OCR viaja codificado)**: `rasterizePage` pasa a devolver `EncodedPageImage` (PNG) en vez de `ImageData`. Medido sobre 50 páginas escaneadas, la aplicación crece entre 335 y 563 MB dentro de la ventana `OCR_STARTED → OCR_FINISHED` (3 de 3 corridas), con el grueso en el proceso del renderer y entre 80 y 216 MB más en el proceso GPU; de eso, ~209 MB son las **tres copias simultáneas** de cada página en vuelo a 300 dpi — el canvas del kernel, el `ImageData` que llegaba al host, y el clon estructurado que cruzaba al worker de OCR. Y había una cuarta: ese worker reconstruía **otro** `OffscreenCanvas` con `putImageData`, después de lo cual **tesseract.js encodeaba ese canvas a PNG igual** (su `loadImage` convierte cualquier entrada a bytes de imagen codificada; su core no acepta píxeles crudos). Movíamos 35 MB por página, los copiábamos cuatro veces, y el consumidor final los comprimía de todas formas. Ahora el encode ocurre **una sola vez**, en el kernel, sobre el canvas que ya existe. Este motor no pierde ninguna capacidad: el camino de preview/export (`renderPage`, `RenderPageOutput.encoded`) no se toca, y `rasterizePage` sigue sin emitir eventos, sin tocar el cache LRU y sin participar del supersede. Ver §6, §10, §13 caso 30, §14, §15.
>
> **Nota (v1.15.0, ADR-144, 2026-09-09 — el input se registra ya; el trabajo se planifica)**: `Orchestrator.seedAnonymizedPreview` llama `renderPage` por cada página del documento sin esperar, y `WorkerPool.enqueue` acepta encolar de más aunque pase `maxQueue` (emite `WORKER_POOL_SATURATED` y encola igual) — en un expediente de 200 páginas eso son ~200 renders lanzados en el mismo turno, sin que el límite documentado limite nada. `rememberInput` (§9, nota de implementación 1) sigue corriendo **sincrónico** al principio de `renderPageInternal`, sin condicionar a nada — posponerlo rompería la reconstrucción de `RENDER_REQUESTED`. Lo que se demora es el trabajo **pesado** —cache check + dispatch al kernel— y **solo para `mode: "preview"`**: `mode: "full"` (export) sigue despachando inmediato, prioridad 1000, bit a bit como siempre. Ese trabajo pasa por `PreviewRenderScheduler` (wiring interno, no exportado desde `index.ts` — mismo criterio que `RenderJobPool`): coalesce por clave `documentId|pageIndex|kind|mode|scale|imageFormat` (sin `replacements`/`annotations`, que es justo lo que cambia entre una ráfaga de ediciones sobre la misma página) y acota la concurrencia a `ctx.config.workerPool.renderPoolSize` despachos de preview **en vuelo** a la vez, con el cupo reservado **antes** de despachar. Una solicitud nueva sobre una clave con trabajo pendiente reemplaza el descriptor (bump de generación + máximo de prioridad) y devuelve la MISMA promesa — no dispara nada nuevo; si el kernel resuelve con una generación vieja, el resultado se descarta sin cachear ni emitir `PREVIEW_UPDATED` y se redespacha leyendo el input **vigente** (`lastAnonymizedInputs`/`lastOriginalInputs`, nunca el snapshot que armó la entrada). Prioridad (la fila que `05_Worker_Architecture.md` §6.2 ya documentaba y el código no implementaba: preview visible=70, no visible=20): la decide `renderPageInternal` a partir del `participatesInSupersede` que ya recibía — `handleRenderRequested` (el visor pidiendo lo que mira) → 70; llamada directa (seed/flush mediado) → 20 — sin parámetro nuevo en la firma pública. El mecanismo de supersede de ADR-037 §4 (`pendingRenders`, checkpoint antes/después del kernel) **no se toca**: son dos coalescings ortogonales — ADR-037 detecta "cambió la escala pedida para esta clave SIN escala"; ADR-144 coalesce por una clave que SÍ incluye `scale`, y solo bajo `mode: "preview"`. Baja de documento: `clearDocumentState`/`dispose` propagan a `previewScheduler.clearDocument`/`.clear()` — un descriptor que sobreviva a un `unloadDocument` es la fuga que el ADR declara en sus Consecuencias; no hay forma de cancelar a mitad de camino un `pool.dispatch` en curso sin una señal nueva que el ADR no pide, así que la baja rechaza (`CancelledError`) e invalida de inmediato, dejando que el kernel en vuelo termine en segundo plano con su resultado descartado. Sin cambios de contrato (`Contracts.md` intacto), sin tocar el camino de export ni la mediación de ADR-044 del lado del Orchestrator. Ver §2, §9, §13 casos 35-36, §14, §15 ítem 31.

> **Nota (v1.14.0, ADR-109, 2026-08-30 — la caja que llega es la de tinta, y el motor no cambia una línea)**: `Word.bbox`, y con él `Replacement.bbox`, deja de ir de la línea de base hacia arriba por un cuerpo entero y pasa a ser la **caja de tinta** de la palabra (`Contracts.md` §5). Este motor **no se toca**: consume el rectángulo que le llega y sigue haciendo exactamente lo mismo. Tres consecuencias que conviene tener presentes al leer el spec:
> - **Tapar el bbox ahora tapa las descendentes.** Antes las colas de `g`, `j`, `p`, `q`, `y`, la `Q` y las comas quedaban fuera del rectángulo en una de cada tres palabras. Es el defecto que este cambio cierra, y se ve en el export.
> - **`REPLACEMENT_FONT_HEIGHT_RATIO` baja de 0,7 a 0,64** (`Contracts.md` §6, ADR-109 §4), y es exactamente la recalibración que hace que `replacementFontSize` y `naturalReferenceSize` den lo mismo que hoy sobre una caja ~1,10 veces más alta. El veredicto de degradación (§13 caso 28) se preserva por la misma aritmética: `DEGRADED_FONT_RATIO` no cambia.
> - **El repintado de línea centra mejor.** `tryRepaintLine` dibuja con `textBaseline: "middle"` en el medio de la caja; ese medio baja ~0,9 pt sobre un cuerpo de 8 pt, **hacia** el centro óptico real. Es una mejora esperada sobre el "token levantado" de `roadmap/Post_Hito10.8_Pendientes.md` §25, no un cierre de ese punto.
...
- **Acotar la concurrencia del trabajo pesado de `mode: "preview"` a `renderPoolSize` despachos en vuelo, coalescido por clave (ADR-144)**: `PreviewRenderScheduler` recibe cada `renderPageInternal` de tipo preview, coalesce solicitudes sobre la misma clave (sin disparar más de un despacho real por ráfaga de ediciones) y solo cachea/emite `PREVIEW_UPDATED` para el despacho que no quedó superado por una solicitud más nueva. `rememberInput` sigue sincrónico, sin pasar por el planificador. `mode: "full"` nunca entra a este mecanismo.
...
export interface RenderPageOutput {
  ...
  readonly pageIndex: number;
  readonly kind: "original" | "anonymized";
  readonly imageData?: ImageData;   // ADR-156: opcional, ausente en la práctica — ningún consumidor de hoy lo puebla, en ningún `mode`
...
export class RenderEngine implements IEngine {
  ...
  // ADR-158 §1: devuelve la imagen YA CODIFICADA (PNG), no píxeles crudos. El
  // `convertToBlob` se hace del lado donde el canvas ya existe; el consumidor
  // (OCR, vía el productor del façade) recibe unos pocos MB en vez de ~35.
  rasterizePage(documentId: string, pageIndex: number, scale: number, ctx: EngineContext, region?: BoundingBox): Promise<EncodedPageImage>;
...
- **Devuelve `EncodedPageImage` con `format: "png"`** (ADR-158 §1). PNG y no JPEG: es **sin pérdida**, y este método alimenta el reconocimiento — ahorrar bytes a costa de calidad de entrada está fuera de lo que ADR-158 autoriza. El `widthPx`/`heightPx` del resultado son los del ráster producido (ya multiplicados por `scale`, y del recorte si vino `region`), y son lo que el consumidor necesita para estimar su costo decodificado.
- El encode lo hace el kernel con `convertToBlob` sobre el canvas que ya tiene en la mano. **No es trabajo nuevo**: es el mismo encode que antes hacía tesseract.js del otro lado de la frontera, después de que alguien reconstruyera un canvas para dárselo (ADR-158, Contexto §2).
- En modo pool corre como job del `RenderPool`. El buffer codificado **se clona**, no se transfiere: el consumidor lo retiene entre reintentos (ADR-079, ADR-158 §5), y a unos pocos MB el ahorro de transferir no compensa el riesgo de dejarlo *detached* en el segundo intento.
...
- **Opciones de fuentes, CMaps y canvas (ADR-053; ampliado)**: ese `getDocument()` lleva además `disableFontFace: true`, `useSystemFonts: false`, `useWorkerFetch: false`, `cMapUrl: "/pdfjs/cmaps/"` + `cMapPacked: true`, `standardFontDataUrl: "/pdfjs/standard_fonts/"`, y **tres** factories propias: CMap, standard fonts y **canvas**. Las seis opciones y las tres factories son **solidarias**, y cada una tapa un agujero distinto del mismo tipo — pdf.js asumiendo un DOM que dentro de un Worker no existe:
  - Omitir `useWorkerFetch: false` hace que pdf.js evalúe `document.baseURI` y tire `ReferenceError` dentro del Worker, que `kernelLoadDocument` reclasifica como `RenderFailedError` — o sea, el visor entero deja de cargar documentos con un error que no menciona fuentes.
  - Omitir la **`CanvasFactory`** hace que pdf.js caiga a su `DOMCanvasFactory`, que llama `document.createElement("canvas")`, y falla con `Cannot read properties of undefined (reading 'createElement')`. pdf.js pide canvas auxiliares solo cuando la página los necesita —**grupos de transparencia, soft masks, patrones de mosaico y fuentes Type3**—, así que una página de texto y vectores no lo dispara nunca: por eso el motor tenía 57 tests de unidad en verde mientras cualquier PDF salido de un convertidor real fallaba en **todas** sus páginas. El detalle medido está en `roadmap/Post_Hito10.8_Pendientes.md` §21. La factory pide su contexto con `willReadFrequently: true`, porque pdf.js relee estos canvas para componer.

  Los dos prefijos de URL son constantes nombradas del kernel, no config (ADR-053 §3); los assets los sirve la app bajo `/pdfjs/` (ADR-053 §4).
...
> Nota (corregida por ADR-156): `PREVIEW_UPDATED.canvasBlobUrl` es `URL.createObjectURL(blob)` creado en el host a partir de los bytes ya codificados (`entry.encoded.bytes`, §12) — **no** del `ImageData` crudo, que en `mode: "preview"` ni siquiera cruza el `postMessage` desde ADR-156. El kernel codifica el PNG/JPEG una sola vez (`convertToBlob`, ADR-034 §3) y el host arma el `Blob`/`blobUrl` desde esos bytes. Esto evita `createObjectURL` en el worker (no siempre disponible) sin depender de transportar el RGBA crudo.
...
RenderPageOutput {
  ...
  pageIndex: number;
  kind: "original" | "anonymized";
  imageData?: ImageData;  // ADR-156: opcional, ausente en la práctica en los dos `mode`
  encoded?: EncodedPageImage; // solo mode "full" (ADR-034 §3)
  durationMs: number;
}
```

`imageData` es opcional desde ADR-156 y ningún consumidor de hoy lo puebla en ninguno de los dos `mode` — se conserva por si algún día hace falta reintroducirlo, no porque algo lo lea. El blob del preview (`PREVIEW_UPDATED.canvasBlobUrl`) **no** sale de acá: lo arma `emitPreviewUpdated` desde `entry.encoded.bytes` (la entrada interna del cache, siempre codificada — §12), nunca desde `imageData`. En `mode: "full"`, `encoded` trae los bytes PNG/JPEG ya codificados (`convertToBlob` donde vive el canvas, `07_Performance_Strategy.md` §10): es lo que consume el `RenderPageProvider` del Orchestrator para el Export (ADR-034 §3).
...
35. **Un `unloadDocument`/`dispose` con trabajos de preview pendientes o en vuelo (ADR-144 §8)**: toda entrada del `PreviewRenderScheduler` cuya clave empiece con el `documentId` que se da de baja (o todas, sin filtro, en `dispose`) se rechaza de inmediato con `CancelledError` y se borra del `Map` interno — sin esperar a que un `pool.dispatch` en vuelo termine, porque no hay forma de cancelarlo a mitad de camino sin una señal nueva que el ADR no pide. El resultado del kernel, cuando eventualmente llega, se descarta sin cachear ni emitir `PREVIEW_UPDATED`: la entrada ya no está, así que es un no-op. Invariante verificable: ningún descriptor con el prefijo de ese `documentId` sobrevive a la baja — "un descriptor que sobreviva a un `closeDocument` es una fuga" (ADR-144, Consecuencias).
36. **Una solicitud de preview coalesce sobre una clave con trabajo en vuelo (ADR-144 §3/§5/§6)**: `documentId|pageIndex|kind|mode|scale|imageFormat` es la clave — **sin** `replacements`/`annotations`, que es justo lo que cambia entre una ráfaga de ediciones sobre la misma página. Una solicitud nueva sobre una clave ocupada no dispara un segundo despacho: bumpea la generación y el máximo de prioridad, y devuelve la MISMA promesa que ya estaban esperando los llamadores anteriores. Si el despacho en vuelo resuelve con la generación ya vieja (alguien coalesció una solicitud más nueva mientras corría), el resultado se descarta **sin cachear ni emitir `PREVIEW_UPDATED`** y se redespacha leyendo el input autoritativo **vigente** (`lastAnonymizedInputs`/`lastOriginalInputs`, nunca el snapshot que armó la entrada) — recién el despacho que no queda superado cachea/emite y asienta la promesa compartida. `mode: "full"` (export) **nunca** entra a este mecanismo: sigue despachando inmediato con prioridad 1000, sin coalescer con nada, bit a bit como antes de ADR-144.
37. **`renderPage`/`renderPages` sin `ImageData` en el output público, en ningún `mode` (ADR-156)**: `output.imageData` es `undefined` tanto en `mode: "preview"` como en `mode: "full"` — el campo quedó opcional para el día en que haga falta reintroducirlo, no porque algo lo puebla hoy. `output.encoded` sigue presente **solo** en `mode: "full"`, sin cambios (caso 20 de `Contracts.md`/ADR-034 §3 no se toca). El blob del preview (`PREVIEW_UPDATED.canvasBlobUrl`) sale de `entry.encoded.bytes`, igual que antes de este ADR — `emitPreviewUpdated` no cambia una línea. Un cache **hit** también devuelve `imageData: undefined`: la entrada interna (`InternalCacheEntry`) nunca lo guardó, para ningún `mode`.
...
| `renderPage returns an encoded image with correct dimensions (ADR-156: imageData is always undefined)` | `contract.test.ts` | contract | caso 37 |
| `full mode output includes encoded bytes with effective format and quality` | `contract.test.ts` | contract | caso 37 |
...
| `rasterizePage returns an EncodedPageImage (png) without emitting events nor touching cache` | `contract.test.ts` | contract | ADR-034 §1 + ADR-158 §1 |
| `rasterizePage throws InvalidInputError when document not loaded or scale <= 0` | `edge.test.ts` | edge | ADR-034 §1 |
| `rasterizePage with a region returns only the cropped image, encoded` | `unit.test.ts` | unit | caso 30 (ADR-065 §5 + ADR-158 §1): `widthPx`/`heightPx` = `region × scale` |
...
| `coalesces concurrent schedule() calls for the same key into a single runJob dispatch, and every caller resolves with the same result` | `preview-scheduler.test.ts` | unit | ADR-144 §3 — la clase en aislamiento |
| `keeps independent entries for different keys — no coalescing across keys` | `preview-scheduler.test.ts` | unit | ADR-144 §3 |
| `bumps priority to the max across coalesced calls` | `preview-scheduler.test.ts` | unit | ADR-144 §7 |
| `bounds in-flight dispatches to maxConcurrency, queueing the rest until a slot frees` | `preview-scheduler.test.ts` | unit | ADR-144 §4 |
| `discards a result whose generation went stale during dispatch, without settling, and redispatches` | `preview-scheduler.test.ts` | unit | ADR-144 §5/§6 — caso 36 |
| `clearDocument rejects pending/in-flight entries under a documentId prefix with CancelledError and removes them` | `preview-scheduler.test.ts` | unit | ADR-144 §8 — caso 35 |
| `clear() does the same for every document, unconditionally (dispose)` | `preview-scheduler.test.ts` | unit | ADR-144 §8 — caso 35 |
| `wakes a reservation blocked waiting for a free slot when the signal aborts, instead of hanging forever` | `preview-scheduler.test.ts` | unit | ADR-144 §4/§8 — el modo de falla que el ADR exige descartar por test |
| `onSettle can be async and is awaited before resolving the shared promise` | `preview-scheduler.test.ts` | unit | ADR-144 §6 |
| `propagates a runJob rejection (e.g. a checkpoint CancelledError) to the shared promise without special handling` | `preview-scheduler.test.ts` | unit | ADR-037 §4 sigue intacto — el rechazo se propaga sin lógica especial nueva |
| `coalesces N rapid renderPage calls for the same preview key into a single dispatch to the pool, and every caller resolves with the same output` | `unit.test.ts` | unit | ADR-144 §3, coalescencia vía la API pública de `RenderEngine` |
| `bounds concurrent preview dispatches to renderPoolSize under a flood of distinct pages — the case that motivated ADR-144` | `unit.test.ts` | unit | ADR-144 §4, el escenario del Contexto §1 (el seed de 200 páginas) |
| `dispatches with priority 70 via RENDER_REQUESTED (visible) and 20 via a direct call (seed/flush mediado)` | `unit.test.ts` | unit | ADR-144 §7 |
| `mode: "full" bypasses the scheduler entirely — two simultaneous full renders of the same page produce two independent dispatches, not one coalesced` | `unit.test.ts` | unit | ADR-144 §2, Decisión — no regresión del export |
| `a stale generation result is discarded without caching nor emitting PREVIEW_UPDATED; only the redispatch (fresh input) does` | `edge.test.ts` | edge | caso 36 |
| `unloadDocument rejects a pending/in-flight preview scheduler entry with CancelledError and leaves no entry for that document` | `edge.test.ts` | edge | caso 35 |
| `dispose rejects a pending/in-flight preview scheduler entry across all documents, unconditionally` | `edge.test.ts` | edge | caso 35 |
...
- [x] 8f. (Posterior al Hito 10.5, PR 2 de ADR-062) Cabral de vuelta del veredicto de degradación: `KernelRenderResult` devuelve las `Degraded` que `paintReplacements` ya detecta, `InternalCacheEntry` las guarda, y `emitPreviewUpdated` las emite en `PREVIEW_UPDATED.degraded` **desde la entrada** — así el cache hit emite el mismo veredicto que el miss (ADR-062 §4). Sin umbral nuevo, sin anotación nueva, sin dibujo nuevo: ADR-058 §7 queda intacto. Los cuatro tests de §14. Grep de control: ningún `emitPreviewUpdated` que arme el payload sin `degraded`.
...
- [x] 30. (ADR-127) El kernel consume `rectsOverlap` de `@anonly/shared` (`Contracts.md` §6) y borra `overlapsBbox`. **Revierte media línea del item 28**, que había decidido no promoverlo por dos razones: *"es otra pregunta"* —cierta, y ADR-127 §3 de la Decisión la sostiene: `rectsOverlap` **no** reemplaza a `sharesVerticalBand`— y *"tiene un solo consumidor"*, que era falsa: `shared` ya tenía la misma AABB escrita en privado (`intersects`, en `words-in-rect.ts`), con la guarda de área cero que esta copia **no** tenía. El único cambio de comportamiento es esa guarda: un bbox de área cero deja de solaparse. No hay ningún caso vivo que lo produzca, así que los tests de los casos 26/28 no se mueven; los que prueban la primitiva viven en `shared`.
...
- [x] 31. (Hito 11, ADR-144, H-09B de la campaña de hardening) `PreviewRenderScheduler` (`preview-scheduler.ts`, wiring interno, no exportado): coalescencia por clave `documentId|pageIndex|kind|mode|scale|imageFormat` con generación y prioridad `Math.max`, semáforo de `renderPoolSize` despachos en vuelo (reserva cancelable, mismo patrón que `LiveImageBudget` de `ocr-engine`), único punto de cachear/emitir (`settleRenderJob`) condicionado a que la generación no haya quedado vieja durante el despacho. `renderPageInternal` se divide en `runRenderKernelJob` (cache check + dispatch, sin cachear/emitir) + `settleRenderJob`; `mode: "full"` los invoca directo, `mode: "preview"` a través del scheduler. `readFreshInputParts` lee `lastAnonymizedInputs`/`lastOriginalInputs` en el momento del despacho (nunca el snapshot que armó la entrada). `clearDocumentState`/`dispose` propagan a `previewScheduler.clearDocument`/`.clear()`. **No** toca el supersede de ADR-037 §4 (`pendingRenders`/`throwIfSuperseded`), la mediación de ADR-044 del lado del Orchestrator, ni `WorkerPool`. Casos 35-36 de §13 y catorce filas nuevas en §14 (`preview-scheduler.test.ts`, la clase en aislamiento; `unit.test.ts`/`edge.test.ts`, el wiring observable vía la API pública de `RenderEngine`).
