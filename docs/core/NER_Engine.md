...
17. **Segundo worker del pool cargando su propio modelo**: cada worker carga el modelo en su primer job y reporta su propio `model-ready`. El motor emite `NER_MODEL_READY` **una sola vez por instancia** (el primero); los siguientes no producen evento. **Desde ADR-135 `NER_MODEL_LOADING` se deduplica igual**: una vez que el modelo quedó listo para la instancia, la carga de un worker posterior no emite. La asimetría anterior dejaba el indicador de "Preparando el detector de nombres…" prendido para siempre — el `LOADING` tardío lo volvía a encender y el `READY` que lo habría apagado ya estaba deduplicado. ~~`NER_MODEL_LOADING` sí se emite por cada reporte de progreso recibido~~ (la segunda carga es rápida igual: sin `Cache Storage` desde ADR-132 §7, el modelo se lee del origen propio, que en el escritorio es un archivo local).
