...
| `external-data converter rejects a wrong source hash and reproduces both output hashes` | `tests/perf/support/` | unit/integration | ADR-179 §2: convierte el modelo fijado, comprueba 352 inicializadores y 1516 nodos; dos ejecuciones idénticas |
| `external-data arm loads through app:// with the real NER worker and no third-party requests` | `tests/perf/` | integration opt-in | caso 32, ADR-179 §4: falta de sidecar o de `NER_MODEL_READY` aborta antes de medir |
| `external-data arm matches exact NER occurrences on the measured synthetic corpus` | `tests/perf/` | quality opt-in | caso 32, ADR-179 §4: página, tipo, valor, normalizado, confidence y geometría; cero diferencias por entidad |
| `packaging A/B alternates order and reports paired time, M1, M2 and post-Ready peak` | `tests/perf/` | perf opt-in | ADR-179 §5: mínimo tres pares, mismo fixture y controles de instrumento; la ausencia de observación se reporta como tal |
