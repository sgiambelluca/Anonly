...
- Traducir el ciclo de vida del modelo reportado por el kernel (`PROGRESS.partial.phase`, ADR-046 §4) a `NER_MODEL_LOADING`/`NER_MODEL_READY`, deduplicando `NER_MODEL_READY` a uno por **ciclo de carga** (ADR-135 + ADR-166 §1bis: la baja del pool reinicia el flag).
...
//
// El puerto interno del pool devuelve `boolean` (`true` solo si terminó
// workers): la guarda de `WorkerPool` puede frenar la baja y el motor
// necesita distinguirlo para no reiniciar `modelWarm` con un worker vivo
// que todavía tiene el pipeline cargado (ADR-166 §1bis, caso 29). La firma
// pública del motor no expone ese booleano.
...
28. **`releaseIdleWorkers()` con el pool ocioso** (ADR-166): termina los `WorkerLike` vivos del pool interno. No es `dispose()` — el motor sigue usable, y el próximo `processPage` reconstruye el worker perezoso (ADR-080) pagando la recarga del modelo. **`releaseIdleWorkers()` reinicia `modelWarm`** (ADR-166 §1bis), así que `isModelReady()` vuelve a `false` y `NerStarted.modelLoading` lo refleja: el flag host-side describe si hay un kernel con el modelo cargado, y después de terminar los workers no lo hay. Un `getModelId()` posterior sigue devolviendo el de la config, que no depende del worker.
29. **`releaseIdleWorkers()` con el pool NO ocioso** (ADR-166): no hace nada, por la guarda de `WorkerPool.releaseIdleWorkers()` — `terminate()` no dispara `error`, así que matar un worker con un job pendiente dejaría esa promesa colgada para siempre. La memoria la libera el temporizador de ADR-080 (60 s) como hasta hoy. Es el caso de una cancelación con un batch todavía en vuelo. **Y `modelWarm` NO se reinicia en este caso** (ADR-166 §1bis): el worker sigue vivo con el pipeline cargado, y el kernel hace early-return sin reportar `model-loading`/`model-ready` sobre un modelo que ya tiene, así que reiniciar el flag lo dejaría en `false` para siempre — con `isModelReady()` y `NerStarted.modelLoading` mintiendo. El motor distingue los dos casos por el `boolean` que devuelve el puerto del pool. Sin `workerFactory` (fallback in-process): no-op inocuo en los dos casos.
30. **Reanálisis después de la baja** (ADR-166 §2): `runReanalyzeNerOnFlow` y `runReanalyzeOcrFlow` vuelven a llamar a `processPages` con el pool ya dado de baja; el worker se reconstruye y el modelo se recarga. Es el **costo declarado** del ADR —942,94 ms medianos en frío— y no un error: no emite `NER_MODEL_LOAD_FAILED` ni ningún error, y el camino es indistinguible de la primera carga del documento — **incluidos sus eventos**: con `modelWarm` reiniciado por la baja, la recarga emite `NER_MODEL_LOADING` y `NER_MODEL_READY` otra vez, que es lo que mantiene el indicador del cliente en hora durante ese segundo (ADR-135, ADR-166 §1bis).
