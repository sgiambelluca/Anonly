...
> **Nota (v1.9.0, ADR-173 — evaluación de empaquetado, sin cambio del motor de producción)**: el punto 1 de recursos compara el ONNX Q8 actual con los mismos pesos dispuestos como grafo `.onnx` + `model_quantized.onnx_data`. La conversión y el brazo con `use_external_data_format: 1` viven en el banco opt-in `tests/perf/`; no cambian `NerConfig`, el asset fijado de producción ni la llamada normal a `pipeline()`. La adopción exige compatibilidad en Electron/Chromium/WASM, igualdad exacta de salida y medición A/B intercalada (§12–§15). `.ort` no entra en este brazo porque el cargador de Transformers.js usado por el kernel pide `.onnx`.

> **Nota (v1.7.0, ADR-123, 2026-09-02 — la `S` de `S/` no es una inicial)**: sobre `SUAREZ, BARTOLOME ARTURO S/ RECURSO DE` el modelo devuelve el span `BARTOLOME ARTURO S` — se lleva la primera letra de `S/`, la partícula que separa a las partes de una carátula. El span está mal desde siempre; lo que lo **destapó** fue ADR-122, y el mecanismo vale la pena anotarlo: `bartolome arturo s` se fusionaba con `bartolome arturo` por **0,889** contra un umbral de 0,88, una milésima, y cuando la carátula absorbió las 16 ocurrencias del grupo bueno (ADR-117) el espurio quedó solo y a la vista, con una caja que tapaba el nombre y dejaba el apellido. Después del encaje de bordes de ADR-111 corre un paso más: **si un span termina en una letra suelta y pegado a ella hay una barra, la letra se descarta**. Las dos condiciones hacen falta — la barra prueba que la letra es del token de al lado (una inicial de verdad va seguida de un **punto**, y sin esta condición la regla borraría `Juan P.`), y que la letra sea un token suelto evita que de `Quilmes/ La Plata` salga `Quilme`. Es un hecho de tokenización, no una interpretación del nombre, y por eso la condición mira el texto que está **afuera** del span. `snapSpansToWordBoundaries` no lo arregla y no podría: `/` no es `WORD_CHAR_RE`, así que para esa función la `S` ya es una palabra entera; ensanchar daría `BARTOLOME ARTURO S/`, que es peor. **El paso corre después del encaje**, y no por lo que la primera versión de ADR-123 decía —el ensanche no puede devolver la letra— sino porque la letra es **ambigua hasta que la palabra está completa**: en `Documento IPS/ 12` es la cola de `IPS`, y recortando antes se pierde la entidad entera. Un span que se queda sin letras ni dígitos se descarta entero. Medido sobre **451** spans de `PERSON` en 8 documentos: **4** terminan justo antes de una barra y los cuatro son este defecto; después del cambio son **0** y siguen siendo 451 spans — no se pierde ninguno, y los 3 corregidos pasan a terminar donde termina la carátula, así que ADR-117 los absorbe y el grupo espurio desaparece sin perder cobertura. Ver §13 caso 27 y §14.
...
- Traducir el ciclo de vida del modelo reportado por el kernel (`PROGRESS.partial.phase`, ADR-046 §4) a `NER_MODEL_LOADING`/`NER_MODEL_READY`, deduplicando `NER_MODEL_READY` a uno por **ciclo de carga** (ADR-135 + ADR-167 §3: **cualquier** baja del pool —por inactividad o explícita— reinicia el flag).
- Marcar ocurrencias con `confidence < NER_CONFIDENCE_THRESHOLD` para que Grouping las marque conflicto `low_confidence`.
- ~~Cache del modelo en Cache Storage del navegador, versionado por `modelId`.~~ **Retirado (ADR-132 §7, 2026-09-04)**: `configureTransformersEnv()` fija `env.useBrowserCache = false`. Ver §12 para el razonamiento y para la condición que lo revierte.
...
> **Nota (ADR-167, 2026-09-18 — reemplaza la de ADR-166)**: el modelo de NER
> —200-400 MB de pesos más su sesión de inferencia (§12)— se suelta cuando su pool
> lleva `nerIdleDisposeMs` sin trabajo (**default 15 s**, `Contracts.md` §6), por el
> temporizador de ADR-080 que vive en el propio `WorkerPool`. **El motor no expone
> ninguna baja propia**: la que agregó ADR-166 —`releaseIdleWorkers()`, invocada al
> cerrar la detección— se retiró junto con sus contadores de guarda, porque T-8
> midió que soltar el modelo en ese punto le trasladaba ~1,2 s y ~500-600 MB de pico
> al documento siguiente (`roadmap/AB_Intercalado_Medicion.md`). Lo que el motor sí
> hace es **enterarse** de cada baja: se suscribe a `onWorkersReleased` del puerto
> del pool y reinicia `modelWarm` (ADR-167 §3). Así la recarga posterior —por
> reanálisis o por un documento nuevo, a los 15 s o cuando sea— emite
> `NER_MODEL_LOADING`/`NER_MODEL_READY` como una primera carga. **Antes de ADR-167 esa
> recarga era muda** en el camino del temporizador: medido, el modelo se recargaba en
> ~2 s sin emitir `NER_MODEL_READY`. Ver §13 casos 28-31.
...
```ts
// Puerto interno del pool — NO es superficie pública del motor. Además de
// despachar, notifica cada baja efectiva de sus workers, venga del
// temporizador de inactividad (`nerIdleDisposeMs`) o de una llamada explícita
// (ADR-167 §3). "Efectiva" = `WorkerPool.releaseIdleWorkers()` devolvió `true`:
...
- Memoria: 200–400 MB por worker (modelo + sesión de inferencia). **Desde ADR-167 esa memoria se suelta a los 15 s de inactividad del pool** (`nerIdleDisposeMs`), en vez de los 60 s de `idleDisposeMs` que siguen usando los demás pools (§6, §13 casos 28-31). T-8 midió ~390 MB liberados desde los 30 s de terminada la detección.
- **Modelo NO cacheado en Cache Storage** (~150–180 MB Q8 para mBERT, ADR-023). Lo estuvo hasta ADR-132 §7. Dos razones para apagarlo, y la segunda es la que manda:
  ...
  2. **No aportaría nada aunque funcionara.** Desde ADR-130 el modelo viaja **dentro del instalador**: es un archivo local. Guardar una copia en `Cache Storage` es copiar del disco al disco para después leer del disco.

  > **Si vuelve un cliente web o una PWA, esto tiene que volver a `true`.** Ahí la caché sí compraba algo: el origen propio era un *servidor*, y la copia local evitaba re-descargar ~180 MB en cada visita y habilitaba el modo offline — que es lo que describía la versión previa de esta línea. Sin ella, una web re-descarga el modelo en cada visita y nada avisa. La constante vive en `configureTransformersEnv()` (`src/worker/kernel.ts`), con la misma nota.
- Sin transferencia zero-copy de `text` (es string, se serializa normal).
- Paralelismo actual: el host procesa páginas y batches secuencialmente, por lo que los despachos usan un solo slot y el segundo worker de `NerPool` no se instancia en el flujo normal, aunque `nerPoolSize` sea 2. La prueba de dos workers se midió y se revirtió (`roadmap/Optimizacion_De_Rendimiento.md` §C). Dentro del worker activo, ONNX Runtime usa su build WASM multihilo cuando el entorno permite `SharedArrayBuffer`; ADR-130/132 verificaron esas condiciones en el escritorio. `numThreads` queda en selección automática del runtime y el número efectivo de hilos depende del entorno. Backpressure si `queue > 8`.
...
- **Evaluación opt-in de empaquetado (ADR-173)**: el control carga el asset ONNX Q8 fijado y el candidato carga el mismo grafo/pesos con datos externos y `use_external_data_format: 1`. El experimento no cambia el código, el modelo ni el presupuesto de producción. El arnés versionado bajo `tests/perf/` genera y verifica el candidato con la herramienta y los hashes de ADR-173 §2; construye ambos brazos desde la misma revisión, prueba primero `app://` + Chromium/WASM, compara las ocurrencias exactas y solo entonces mide carga, M1, M2, pico posterior a `Ready` y nivel sostenido mediante pares intercalados. El loader local probado en Node no sustituye ese gate. Una señal inconclusa o una diferencia de detección conserva el control. El default solo puede cambiar tras el informe y la enmienda de adopción de ADR-173.
- **Cuándo se carga el modelo, y por qué no antes**: la carga ocurre dentro del primer batch de NER, o sea después de que termina el OCR del documento. Adelantarla para solaparla con el OCR se midió en tres rondas intercaladas y se descartó: no dio una mejora estable de `import→Ready`, subió el pico de RSS 144–422 MB y retrasó el OCR (`roadmap/Precalentamiento_NER_Durante_OCR_Medicion.md` §7, anotado en ADR-154 §2 lever 3). Cargarlo al abrir la aplicación también está descartado: el pool libera el worker a los 60 s de inactividad (ADR-080), así que el modelo se descargaría antes de usarse en cualquier sesión donde el usuario tarde en importar, y mientras tanto ocupa los 200–400 MB de arriba sin documento abierto. El orden es OCR → NER a propósito, no por omisión.
...
17. **Segundo worker del pool cargando su propio modelo**: cada worker carga el modelo en su primer job y reporta su propio `model-ready`. El motor emite `NER_MODEL_READY` **una sola vez por ciclo de carga** (el primero); los siguientes no producen evento. **Desde ADR-167 el ciclo se reabre con cualquier baja del pool** (§6): el pool la notifica por `onWorkersReleased`, el motor reinicia el flag, y la recarga siguiente vuelve a emitir el par `LOADING`/`READY`. Lo que este caso fija —que el segundo worker calentándose no emite— no cambia. **Desde ADR-135 `NER_MODEL_LOADING` se deduplica igual**: una vez que el modelo quedó listo para la instancia, la carga de un worker posterior no emite. La asimetría anterior dejaba el indicador de "Preparando el detector de nombres…" prendido para siempre — el `LOADING` tardío lo volvía a encender y el `READY` que lo habría apagado ya estaba deduplicado. ~~`NER_MODEL_LOADING` sí se emite por cada reporte de progreso recibido~~ (la segunda carga es rápida igual: sin `Cache Storage` desde ADR-132 §7, el modelo se lee del origen propio, que en el escritorio es un archivo local).
...
28. **El pool libera por inactividad** (ADR-167 §2-§3): tras `nerIdleDisposeMs` sin trabajo, el temporizador del pool termina sus `WorkerLike` y notifica a los suscriptos de `onWorkersReleased`. El motor reinicia `modelWarm`: `isModelReady()` vuelve a `false` y `NerStarted.modelLoading` lo refleja, porque el flag host-side describe si hay un kernel con el modelo cargado y ya no lo hay. No es `dispose()`: el motor sigue usable y el próximo `processPage` reconstruye el worker perezoso. `getModelId()` sigue saliendo de la config. El pool de NER usa `nerIdleDisposeMs`, **no** `idleDisposeMs` (ADR-167 §5.2).
29. **El temporizador vence con el pool NO ocioso** (ADR-167 §5.4): no hay baja ni notificación. La guarda de `WorkerPool.releaseIdleWorkers()` (ADR-080) nunca termina un worker con un job en vuelo —`terminate()` no dispara `error` y esa promesa quedaría colgada para siempre— y devuelve `false`. **Y `modelWarm` NO se reinicia**: el worker sigue vivo con el pipeline cargado, y el kernel hace early-return sin reportar `model-loading`/`model-ready` sobre un modelo que ya tiene, así que reiniciar el flag lo dejaría en `false` para siempre, con `isModelReady()` y `NerStarted.modelLoading` mintiendo (el hallazgo de ADR-166 §1bis, que sigue vigente). En la práctica el temporizador se cancela al entrar un job y se rearma al quedar ocioso, así que este caso solo lo alcanza una carrera; la guarda es lo que lo vuelve inocuo.
...
| **`an entity split across two lines emits one occurrence with two fragments`** | `unit.test.ts` | unit | ADR-074 §2 — **el test que define el ADR** de este lado, y el motor donde se midió: `Diego` / `Ramos Vargas` en renglones distintos |
...
| `drops a trailing single letter that belongs to a caption separator` | `worker/__tests__/kernel.test.ts` | unit | caso 27 (ADR-123) — el caso real del encabezado escaneado; la aserción sobre `normalizedValue` es la que evita el grupo espurio |
| `keeps a real initial, which is followed by a dot and not by a slash` | `worker/__tests__/kernel.test.ts` | unit | caso 27 — la mitad de la condición que mira la barra |
| `does not touch a span that ends before a slash without a lone letter` | `worker/__tests__/kernel.test.ts` | unit | caso 27 — la otra mitad: sin ella, `Quilmes/` → `Quilme` |
| `drops the span entirely when the separator letter was all of it` | `worker/__tests__/kernel.test.ts` | unit | caso 27 — un span sin letras no nombra a nadie |
| `runs after the boundary snap, so a word tail is not mistaken for a separator` | `worker/__tests__/kernel.test.ts` | unit | caso 27 — **el único test que fija el ORDEN**: con el recorte antes del encaje, `Documento IPS/ 12` pierde la entidad `IPS` entera |
...
| `external-data converter rejects a wrong source hash and reproduces both output hashes` | `tests/perf/support/` | unit/integration | ADR-173 §2: convierte el modelo fijado, comprueba 352 inicializadores y 1516 nodos; dos ejecuciones idénticas |
| `external-data arm loads through app:// with the real NER worker and no third-party requests` | `tests/perf/` | integration opt-in | caso 32, ADR-173 §4: falta de sidecar o de `NER_MODEL_READY` aborta antes de medir |
| `external-data arm matches exact NER occurrences on the measured synthetic corpus` | `tests/perf/` | quality opt-in | caso 32, ADR-173 §4: página, tipo, valor, normalizado, confidence y geometría; cero diferencias por entidad |
| `packaging A/B alternates order and reports paired time, M1, M2 and post-Ready peak` | `tests/perf/` | perf opt-in | ADR-173 §5: mínimo tres pares, mismo fixture y controles de instrumento; la ausencia de observación se reporta como tal |
...
- [ ] 5. ~~Implementar `init` (crear `NerPool`, …)~~ **Superseded por ADR-046 §2**: el motor no crea su pool — la recibe por constructor (`new NerEngine(pool?)`); la carga del modelo Q8 vive en el kernel, perezosa en su primer job (el cache en Cache Storage se retiró en ADR-132 §7).
...
- [x] 32. (ADR-173 — punto 1 de recursos, opt-in) En `tests/perf/` y su soporte: fijar la herramienta offline y la conversión exacta de ADR-173 §2, verificar los tres hashes, crear los brazos A/B desde la misma revisión con un único parche de opción de carga, gatear carga real `app://`/Chromium/WASM y calidad exacta antes de medir, y ejecutar el A/B intercalado de ADR-173 §5. Entregar resultados crudos y resumen al planificador para el informe de medición. **No** cambiar `NerConfig`, `Contracts.md`, `assets.lock.json`, el asset normal ni el default del kernel durante esta evaluación. Informe: `roadmap/Empaquetado_NER_Medicion.md`; se conserva A por señal RSS insuficiente.
...
- `adr/ADR-173-El-Empaquetado-De-NER-Se-Evalua-Sin-Cambiar-El-Modelo.md` (experimento de empaquetado del mismo ONNX, con gates antes de adoptar)
