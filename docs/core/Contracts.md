 ...
 * `03_Data_Model.md` §5.
 *
 * ADR-109: la caja de una palabra es su **caja de tinta** — va del descenso
 * al ascenso de su fuente (`base − |descent|·cuerpo` … `base + ascent·cuerpo`),
 * no de la línea de base hacia arriba por un cuerpo entero. El tipo NO cambia
 * de forma; lo que cambia es qué significan `y` y `height` para una palabra
 * de `source: "pdf"`, y el cambio los pone de acuerdo con lo que
 * `source: "ocr"` ya significaba (ADR-064). Consecuencia para quien pinta:
 * tapar `bbox` tapa las descendentes, que antes quedaban afuera en una de
 * cada tres palabras.
 */
...
/*
 * ADR-074 §1 — una entidad puede cruzar un salto de línea, y entonces UN
 * rectángulo no alcanza: la unión de sus palabras es la envolvente, que
 * abarca las dos líneas enteras (medido: 557,2 × 18,2 pt sobre una pericia
 * real). La descomposición por línea viaja en un campo aparte,
 *
 *   readonly fragments?: ReadonlyArray<BoundingBox>;   // ausente ≡ [bbox]
 *
 * que llevan `Occurrence`, `OccurrenceRef` y `Replacement`
 * (`03_Data_Model.md` §7, §8, §12) — NO va adentro de `BoundingBox`: un
 * rectángulo no contiene rectángulos, y `bbox` sigue siendo la envolvente
 * para orden documental (ADR-028), solapamiento, hit-test y miniatura.
 * Quien PINTA usa `fragments ?? [bbox]`, nunca la envolvente sola.
 * No hay tipo nuevo que declarar acá (§10 regla 1): es un campo opcional
 * sobre tipos que ya existen, con el mismo patrón que `rotation` arriba.
 */
...
>
> El **valor** bajó de 0,6 a 0,5 en el mismo movimiento porque el número viejo medía otra magnitud; con 0,6 el criterio nuevo marcaría placeholders normales en cajas apretadas. Y la **invariancia de escala** —el motivo por el que esto es una razón y no un piso en píxeles— pasa a ser exacta: el tamaño de referencia no lleva piso ni redondeo, y el piso de dibujo escala con el render. Eso es lo que hace que el veredicto del preview valga para el PDF exportado.
...
>
> **`buildOccurrenceContext(text: string, startIndex: number, endIndexExclusive: number): OccurrenceContext | undefined`** y **`OCCURRENCE_CONTEXT_CHARS = 40`** (ADR-105) — la frase que rodea a una ocurrencia, para que el separador pueda distinguir apariciones del **mismo** valor: dos personas que se llaman igual, o un topónimo usado como ciudad y como nombre de un organismo. Devuelve `before`/`after` como **dos cadenas** —la UI arma `…{before}` **{value}** `{after}…` sin aritmética— y el valor **no** se repite adentro. Ventana de caracteres a cada lado, recortada al límite de palabra; **no** corte por oración, porque partir oraciones en español jurídico es un problema propio (`Dr.`, `N°`, `art.`, `I.P.P.` son puntos que no terminan nada). `undefined` cuando no hay nada alrededor o el rango es imposible, para que el consumidor distinga "sin contexto" de "contexto vacío". Vive en `shared` y no en cada motor por la misma razón que las dos de abajo: `regex-engine` y `ner-engine` la necesitan igual y no pueden importarse entre sí (P-1/P-2).
...
>
> **`startWorkerEntry(definition: WorkerEntryDefinition): void`** y **`postWorkerMessage(message, transfer?)`** (ADR-128) — el **esqueleto del `worker/entry.ts` de un motor**, en `@anonly/shared`. Instala el ciclo de vida completo del entry-point: `WORKER_ID` con sufijo aleatorio, el `Map<signalId, AbortController>`, el guard de `jobType`, el mapeo de un `RUN` a `COMPLETED` / `CANCELLED` / `FAILED` (con `err.serialize()` si es `EngineError`, y envuelto en `InvalidInputError` conservando el mensaje si no lo es), la limpieza del `Map`, el switch `INIT`/`RUN`/`CANCEL`/`DISPOSE` y el `READY` eager. Los puntos de variación viajan en la `definition`: `applyConfig` (que **puede devolver una promesa**, y entonces el `READY` espera a que resuelva — el caso de `pdf-engine`, que espera a su `engine.init()`), `run(payload, ctx)`, `onCancel` (trabajo extra al cancelar, además de abortar — `export-engine` descarta ahí su PDF parcial), `ready` (una promesa que el `READY` **eager** espera, para `pdf-engine`, que no puede declararse listo antes que su motor), `dispose`, `transferablesOf` y las `capabilities`. `ctx` da `abortSignal`, `jobId` y `progress()`. `postWorkerMessage` es la válvula para lo que vive **fuera** del ciclo de un job —hoy solo el puente de `EVENT`/`LOG` de `pdf-engine`—. **Este módulo no importa nada de Web Workers**: toca `self` dentro del cuerpo de las funciones, nunca a nivel de módulo, así que importarlo desde el hilo principal no ejecuta nada. Estaba escrito cinco veces y ya había divergido (la firma de `post()` no era la misma en los cinco).

> **`rectsOverlap(a: BoundingBox, b: BoundingBox): boolean`** (ADR-127) — predicado puro de `@anonly/shared`: `true` cuando los dos rectángulos comparten **área**. **Estricto** (tocarse por el borde exacto no cuenta, mismo criterio que `sharesVerticalBand`) y **un rectángulo sin área no se solapa con nada**. Estaba escrito dos veces —`intersects`, privada de `words-in-rect.ts`, y `overlapsBbox` en el kernel de `render-engine`—, con la misma AABB y solo una de las dos con la guarda de área cero; llega acá por el mismo camino y con el mismo argumento que `sharesVerticalBand` y `normalizeForComparison` (ADR-061 §2 errata). **No reemplaza a `sharesVerticalBand`**, que mira solo el eje Y: son dos preguntas distintas y tienen que poder responder distinto (ADR-127 §3). Tampoco a los ratios de solapamiento de `grouping-engine` y `pdf-engine`, que comparten la aritmética pero son **reglas de negocio distintas** (denominadores distintos, calibrados por separado).

> **`normalizeForComparison(value: string): string`** (ADR-061 §2, errata) — función pura de `@anonly/shared`: NFC, minúsculas, sin diacríticos (marcas combinantes `U+0300`–`U+036F` sobre la forma NFD), `trim` y colapso de espacios repetidos. Es la normalización de comparación de **texto libre** —nombres, direcciones—, distinta de los `normalizer` por tipo de dato de `RegexPattern` (`stripDots`, `stripDashes`), que asumen un formato estructurado. La usan `grouping-engine` (claves del léxico de género, ADR-060 §4: el build script y el lookup de runtime tienen que coincidir) y `regex-engine` (`findLiteral`). Incluye `trim` y colapso **a propósito**: es lo que hace que los dos consumidores la usen tal cual, sin wrapper. **Ya no produce el `normalizedValue` de las ocurrencias manuales** — eso pasó a `normalizeEntityValue` (ADR-115 §1).

> **`normalizeEntityValue(value: string): string`** (ADR-115 §1) — función pura de `@anonly/shared`: `normalizeForComparison` **más el recorte de todo lo que no sea letra ni dígito en los bordes** (`/^[^\p{L}\p{N}]+|[^\p{L}\p{N}]+$/gu`). Es la normalización de **`Occurrence.normalizedValue`**, o sea la clave con la que `grouping-engine` agrupa. Existe separada de `normalizeForComparison` porque ésa es además la del léxico de género, donde build y runtime tienen que coincidir carácter a carácter (ADR-060 §4) y el recorte no aporta nada. Solo los **bordes**: `A.C.` → `a.c`, `Empresa S.A.` → `empresa s.a`, un CUIT con guiones queda intacto. Sin el recorte, el mismo apellido llega con la puntuación que le tocó al lado —`SUAREZ,` en un sello, `“SUAREZ,` en una carátula, `Suarez.` al final de una oración— y sale con tres claves que el pase difuso de grouping no junta (0,857 y 0,750 contra un umbral de 0,88): medido, un solo apellido abría cuatro grupos.
>
> **Es la única definición de `normalizedValue`** (ADR-118): `ner-engine` también la usa, en vez de la copia local que tenía. Esa copia no sacaba diacríticos, así que el mismo nombre llegaba con dos claves según qué detector lo encontró — medido, de 108 ocurrencias con diacríticos **23 se partían en dos grupos**, con `muñíz`/`muniz` en 0,600 contra un umbral de 0,88.
>
> **`DEGRADED_FONT_RATIO` es una razón, no un tamaño en píxeles, y eso es deliberado** (ADR-058 §7): el preview y el export renderizan a escalas distintas, así que un piso absoluto haría que discreparan sobre si hay que avisar por el mismo reemplazo. La razón es invariante a la escala — y desde **ADR-086 §2** lo es de forma exacta, no aproximada. El piso de fuente mínima se conserva, pero es un piso de **dibujo** y nunca un criterio de aviso: por eso no entra en el tamaño de referencia contra el que se mide, y por eso escala con el render igual que todo lo demás que se dibuja.
>
