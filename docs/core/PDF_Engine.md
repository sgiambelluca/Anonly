...
- `document.pages[i].words` está ordenado por **línea de base** asc (`bbox.y + bbox.height`), luego `bbox.x` asc (ADR-109 §3; hasta entonces la clave era `bbox.y`, equivalente mientras toda caja terminaba en la línea de base).
...
- **Orden de lectura con runs rotados (ADR-067)**, en `sortWordsByReadingOrder` — la única función que fija el orden de `Page.words` y, con él, el de `Page.text`:
  - Un `Word` con `bbox.rotation` **ausente o `0`** se ordena por **línea de base** asc (`bbox.y + bbox.height`), luego `bbox.x` asc, con tolerancia de misma-línea de 1. Una página sin texto rotado produce el array **idéntico** palabra por palabra, así que el snapshot no se regenera (si cambia, el cambio rompió texto horizontal).
  - La clave es la línea de base y no `bbox.y` **desde ADR-109 §3**, porque con la caja de tinta el techo depende del ascenso de cada fuente: en una pericia conviven ascensos 0,688 y 0,905 y sobre un cuerpo de 8,09 pt eso son 1,76 pt de diferencia **en la misma línea**, por encima de la tolerancia — el comparador partiría la línea al medio, que es la regresión que ADR-067 §4 documentó por otra vía. El borde inferior no tiene el problema: era exactamente la línea de base antes de ADR-109 y pasa a ser `base + descent·cuerpo`, con `descent` variando 0,205–0,218 dentro de un documento (0,1 pt sobre 8 pt). **Sobre la geometría previa a ADR-109 el cambio de clave es un no-op demostrable**, y por eso se implementa y se verifica antes que la caja nueva.
  - El orden **entre runs rotados** sigue comparando el ancla por `bbox.y` (ADR-067 §3). Ahí el borde inferior no es una línea de base sino el extremo del run.
...
| `reading order keys on the baseline, not on the box top` | `unit.test.ts` | unit | 11 | ADR-109 §3: dos fuentes de distinto ascenso en la misma línea |
| `keying on the baseline is a no-op on the em box` | `unit.test.ts` | unit | 11 | ADR-109 §3: el paso que se verifica antes del cambio de caja |
| `rotated runs keep ordering by the anchor's y` | `unit.test.ts` | unit | 11 | ADR-109 §3: ADR-067 §3 no se toca |
...
- `adr/ADR-109-La-Caja-De-Una-Palabra-Es-Su-Caja-De-Tinta.md` (la caja va del descenso al ascenso; clave de orden de lectura en la línea de base)
