...
- Los `TextItem` que devuelve PDF.js se dividen por whitespace en `Word`s individuales. El desplazamiento y el ancho de cada token salen de la **tabla de avances reales por glifo** del run que le corresponde (ADR-097 §1), empalmado por cadena exacta + origen (ADR-097 §2). Cuando no hay run que case, el avance se **prorratea linealmente** por longitud de caracteres respecto del `TextItem` original (ADR-020 §1, camino de reserva de ADR-097 §3). En los dos casos el desplazamiento corre sobre el **eje de avance** del run, no sobre `x` (ADR-063 §3): para texto horizontal las dos formulaciones son idénticas.
...
42. **Item multi-palabra empalmado con su run**: el desplazamiento y el ancho de cada token salen de la tabla de avances (ADR-097 §1). Es el caso que motivó el ADR: con el prorrateo, `Juan` caía 8,25 pt a la derecha de su primer glifo y el export mostraba `Ju[HOMBRE 01]`.
43. **Item cuya cadena no coincide con ningún run** (pdf.js sintetizó un espacio, resolvió un `/ActualText` o normalizó): **no** se empalma y el item usa el prorrateo de ADR-020 §1. El bbox es el de siempre; la divergencia se cuenta en el log de empalme (ADR-097 §5), no como error.
