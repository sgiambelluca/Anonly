...
> **Nota (v1.16.0, ADR-140, 2026-09-09 — una página rotada bloquea el documento)**: `ADR-063 §7` dejó registrada la contradicción: Render afirma que las cajas están en coordenadas de página ya rotada, mientras `parsePage` usaba `item.transform` crudo, sin componer la transformación del viewport. Medido con un fixture sintético de cuatro PDFs que solo difieren en `/Rotate` (0/90/180/270): en 0° el error es exactamente cero, y a 90°/180°/270° va de 130 a 268 pt en una página de 200×300 pt — no una caja corrida, una caja en otra parte de la hoja, y a 90°/270° directamente fuera de página (`y = −60`). La causa es que `viewport.width`/`viewport.height` (que van a `Page.width`/`Page.height`) salen de `getViewport({ scale: 1 })`, que ya aplicó `/Rotate`, mientras las posiciones salen de `item.transform`, que no; el volteo `pageHeight - y` usa entonces la altura del marco equivocado. `parsePage` pasa a rechazar con `PDF_PAGE_ROTATED` (`PdfPageRotatedError`, no retryable) toda página cuyo `pageProxy.rotate` —que ya llega resuelto y normalizado por pdf.js, heredado del árbol de páginas— sea distinto de 0 **y** produzca al menos una palabra nativa (content stream o anotación, ADR-066 §1). Una página rotada **sin** texto nativo no entra: va entera por OCR, cuyo `rasterizePage` también usa `getViewport()`, así que el ráster que recibe Tesseract ya está rotado y sus cajas viven en el mismo marco que `Page.width`/`Page.height` — ese camino es consistente y no se toca. Se rechaza el **documento completo**, no solo la página: un expediente "anonimizado" al que le falta la hoja que trae el sello o la carátula es exactamente el resultado con cara de éxito que ADR-126 §2 prohíbe. **Contención, no solución**: ADR-141 es la que retira este guard, por ángulo y solo para los que su corpus deja verificados de punta a punta. Ver §12, §13 casos 62-63, §14, §15 ítem 33.

> **Nota (v1.15.0, ADR-142, 2026-09-09 — una palabra partida entre dos items sigue siendo una palabra)**: `Post_Hito10.8_Pendientes.md` §24 atribuía la fuga de los 8 items del sello de notificación electrónica a un origen mal reportado por `getTextContent()`. Reproducido contra el motor real con PDFs sintéticos escritos a mano (`pdf-lib` no genera la forma), el origen estaba bien: un kerning grande dentro de un mismo `TJ` —el efecto tipográfico de un sello o una carátula— hace que el extractor de pdf.js corte el item en dos **sin insertar ningún espacio** (`compareWithLastPosition` → `flushTextContentItem`, medido contra `pdfjs-dist@4.10.38`). `convertTextItemsToWords` tokenizaba cada item por separado y no tenía ninguna noción de continuidad entre uno y el siguiente: el nombre del caso motivador llegaba al motor como `"N"` y `"otificado"`, dos `Word` en vez de uno, y ningún detector reconocía ninguno de los dos fragmentos. El flujo de glifos continuo de ADR-102 ya tenía la respuesta: incluye los glifos de espacio (`appendRunGlyphs` no filtra nada), así que el motor ya sabe con exactitud si entre el último glifo de un item y el primero del siguiente hay algo. El último token del item A y el primero del item B se empalman en una sola palabra cuando los dos alinearon contra el flujo (ADR-102 §2), el primer glifo de B es exactamente el siguiente del último de A en el flujo (un glifo en el medio cancela el empalme), ninguno de los dos strings termina/empieza en espacio, comparten el mismo versor de avance y de ascenso, el desplazamiento transversal es ≤ 1 pt (`SAME_LINE_TOLERANCE`, el del orden de lectura) y el hueco a lo largo del avance es menor que `0,102 × height` (`TRACKING_SPACE_FACTOR`, la misma constante con la que pdf.js decide que un hueco no es un espacio). El texto resultante concatena sin separador y normaliza NFC **después** de concatenar; el bbox es la envolvente axis-aligned de las cajas de tinta (ADR-109) de los fragmentos. La regla es transitiva —se pliega de a pares— y **no** es simétrica: el caso espejo (dos palabras que pdf.js fusionó comiéndose el espacio real) queda deliberadamente sin resolver, porque cortar en cada glifo en blanco del flujo rompería los runs con `Tw` bajo (ADR-108 §1: 96 espacios en el flujo, 7 con word spacing). El peor caso aceptado es fusionar dos palabras distintas separadas por el mismo kerning y sin espacio real —el dato del archivo no distingue ese caso de una palabra partida—, que tapa de más, nunca de menos. Medido con los cinco casos sintéticos de ADR-142, Contexto §2: el motivador (`"N"` + `"otificado a JUAN PEREZ"`, kerning −4,8 pt) reconstituye `"Notificado"`, un kerning chico que pdf.js no corta (−0,72 pt) sigue igual que antes, y las dos filas de control (kerning positivo, que pdf.js ya resuelve con un espacio sintetizado; e items en líneas distintas) no se empalman. Ver §12, §13 casos 58-61, §14, §15 ítem 32.
...
58. **Un `TJ` con un ajuste de kerning grande dentro del mismo array** (ADR-142 §1): pdf.js corta el item en dos sin insertar espacio y el último token del primero se empalma con el primero del segundo si las seis condiciones se cumplen. Es el caso motivador: `"N"` + `"otificado a JUAN PEREZ"` → `"Notificado"`, `"a"`, `"JUAN"`, `"PEREZ"`.
59. **Tres o más items partiendo la misma palabra**: la regla se pliega de a pares — empalmar A con B y el resultado con C da la misma palabra completa que empalmar los tres a la vez (ADR-142 §1, transitividad).
60. **Dos items glifo-adyacentes en renglones distintos** (mismo `TJ`... salvo por un `Td`/`Tm` de por medio que los separa más de 1 pt en el eje transversal): no se empalman, aunque el índice de glifo sea consecutivo — la condición 5 de ADR-142 corta antes de llegar a la 6.
61. **Dos palabras distintas separadas por el mismo tipo de kerning grande, sin espacio real entre ellas** (`[(JUAN) 400 (PEREZ)]`): se empalman en `"JUANPEREZ"` aunque sean dos palabras — el dato del archivo no distingue este caso de una palabra partida, y el peor caso aceptado es tapar de más (ADR-142, Consecuencias).
62. **Página con `/Rotate` 90/180/270 (heredado o propio) y al menos una palabra nativa** (content stream o anotación): `parsePage` lanza `PdfPageRotatedError` antes de construir la `Page` — el documento entero se rechaza, no solo esa página (ADR-140 §2/§3).
...
| `un kerning grande que pdf.js corta en dos items reconstituye la palabra completa` | `split-word-merge.test.ts` | unit | 11 | caso 58 (ADR-142 §1): `"N"` + `"otificado a JUAN PEREZ"` → `"Notificado"` |
| `es el caso que motivó el ADR: un nombre no queda partido en dos fragmentos sin detectar` | `split-word-merge.test.ts` | unit | 11 | caso 58: ningún fragmento suelto (`"N"` ni `"otificado..."`) sobrevive |
| `un kerning chico, que pdf.js NO corta, sigue tokenizando igual que antes` | `split-word-merge.test.ts` | unit | 11 | no regresión: item único multi-token, camino de ADR-097/102 intacto |
| `tres items partiendo la misma palabra se pliegan de a pares (transitividad)` | `split-word-merge.test.ts` | unit | 11 | caso 59 (ADR-142 §1) |
| `la caja fusionada envuelve a los dos fragmentos` | `split-word-merge.test.ts` | unit | 11 | ADR-142 §2: envolvente axis-aligned de las cajas de tinta |
| `dos palabras distintas, pegadas por el mismo kerning y sin espacio real, se fusionan mal a propósito` | `split-word-merge.test.ts` | unit | 11 | caso 61 (ADR-142, Consecuencias): tradeoff aceptado |
| `cuando pdf.js ya sintetiza el espacio real (kerning positivo), no hay empalme que hacer` | `split-word-merge.test.ts` | unit | 11 | control: un solo item, ya separado por el camino existente |
| `dos items glifo-adyacentes en líneas distintas no se empalman (condición 5)` | `split-word-merge.test.ts` | unit | 11 | caso 60 (ADR-142 §1, condición 5) |
| `un espacio real entre dos items (item en blanco) no se confunde con una palabra partida` | `split-word-merge.test.ts` | unit | 11 | control: `Td` normal entre dos palabras |
| `rechaza una página con /Rotate 90/180/270 y texto nativo` | `page-rotation-guard.test.ts` | unit | 11 | caso 62 (ADR-140 §2): `PdfPageRotatedError`, `code`/`retryable`/`details` exactos |
| `no rechaza una página con rotate === 0 y texto nativo normal` | `page-rotation-guard.test.ts` | unit | 11 | no regresión |
| `no rechaza una página rotada SIN texto nativo — va entera por OCR` | `page-rotation-guard.test.ts` | unit | 11 | caso 63 (ADR-140 §3): `requiresOCR`, `textlessPages` |
| `rechaza una página rotada cuyo único texto nativo viene de una anotación` | `page-rotation-guard.test.ts` | unit | 11 | ADR-140 §2: "content stream o anotación" |
| `una página rotada en cualquier posición aborta la extracción completa del documento` | `page-rotation-guard.test.ts` | unit | 11 | ADR-140 §3: documento entero, no solo la página |
| `raw horizontal text on a /Rotate 90 page composes to a 270°-labeled vertical bbox` | `unit.test.ts` | unit | 11 | caso 64 (ADR-141 §2/§3): página `/Rotate` genuino simulada vía `viewport.transform`, guard de ADR-140 aislado a propósito (ver comentario del describe) |
| `bbox rotation is populated only for right angles` (caso `Rot90`, `dir` crudo vertical sobre página `rotate: 0`) | `unit.test.ts` | unit | 11 | caso 65 (ADR-141 §3, control discriminante): sin la negación de `deriveRotation`, este caso da `270` en vez de `90` |
...
- [x] 32. (Hito 11 — ADR-142) `convertTextItemsToWords`: la alineación contra el flujo de glifos se calcula para **todo** item, no solo los de más de un token (§3 del ADR: sin eso el empalme no se dispara nunca en el caso motivador, un item de un solo carácter). Se agrega el empalme de la cola de un item con la cabeza del siguiente cuando las seis condiciones de ADR-142 §1 se cumplen; el resultado concatena texto (NFC después de concatenar) y toma la envolvente de las cajas de tinta. **No** tocar `Contracts.md` (sin tipo/evento/código nuevo), el camino de reserva de ADR-020 §1 ni la alineación de ADR-102, que se reusa tal cual. Instrumentación nueva (`joinStats.merged`) logueada en `debug` junto a la de ADR-097 §5. Casos 58-61 de §13 y nueve filas nuevas en §14 (`split-word-merge.test.ts`, PDFs sintéticos escritos a mano — `pdf-lib` no genera la forma que dispara el corte).
- [x] 33. (Hito 11 — ADR-140) `PdfPageRotatedError` en `pdf.errors.ts` (`code: PDF_PAGE_ROTATED`, `engineId: Pdf`, `retryable: false`, `details: { documentId, pageIndex, rotation }`). En `parsePage`, después de calcular `contentWords`/`annotationWords` y antes de construir `sortedWords`/`Page`: si `pageProxy.rotate !== 0` y `(contentWords.length > 0 || annotationWords.length > 0)`, lanza. **No** dispara sobre una página rotada sin texto nativo (§13 caso 63: va entera por OCR). El commit del contrato (`Contracts.md` §4, `shared/src/enums.ts`, la fila de este §11) fue **aparte**, con el ADR (R-2/R-19). Casos 62-63 de §13 y cinco filas nuevas en §14 (`page-rotation-guard.test.ts`, siete tests — la primera fila cubre los tres ángulos parametrizados).
- [x] 34. (Hito 11 — ADR-141) `viewport.transform` se compone, exactamente una vez, en `convertTextItemsToWords` (origen y parte lineal de `item.transform`, vía `composeMatrix`/`composeVector` nuevos), en el flujo de glifos (ADR-102), en los orígenes corregidos (ADR-068) y en los rectángulos de imagen (ADR-065/ADR-066 §1). El volteo `pageHeight − yMax` de ADR-063 §2 se retira: `viewport.transform` ya lo incluye. `deriveRotation` mide `atan2(−dir.y, dir.x)` sobre el `dir` YA compuesto — la negación es la línea que hace que la etiqueta sea el ángulo visual de ADR-090 §4 y no su opuesto; es la **única** función que puebla `rotation` (vía `boundingBoxFromParallelogram`), así que el arreglo no se replica. **No** toca `advanceStartOf`/`advanceEndOf`/`compareAlongAdvance`/`crossAxisOf` (ADR-067), `toUprightFrame`, el pintado rotado de ADR-066 §7, `bbox.rotation` de OCR, ni el guard de ADR-140 (sigue lanzando, sin condicionarlo a esta composición). Casos 64-65 de §13, dos filas nuevas en §14, y `03_Data_Model.md` §6 gana la definición normativa del marco de coordenadas.
