...
- **Texto de anotaciones (ADR-066 §1-§4)**, del mismo operator list de la compuerta 1:
  ...
  - **El `rect` del segundo argumento de `beginAnnotation` es el oráculo**: todo word extraído debe caer adentro. El que no, se **descarta con `warn`** — si la composición falló, la posición no es confiable, y una caja negra mal puesta destruye contenido y esconde el error (ADR-066 §3).
  - Se leen todas las anotaciones con texto **salvo** las marcadas `Hidden`/`NoView`: lo que no se dibuja no puede filtrarse por el export. **No** se filtra por `subtype` (ADR-066 §4).
...
31. **Word extraído fuera del `rect` de su anotación**: se descarta con `warn`; no entra en `Page.words` y no se recorta al rect (ADR-066 §3).
...
| `words outside the annotation rect are dropped with a warning` | `edge.test.ts` | edge | 10.8 | caso 31 (ADR-066 §3): no se recortan al rect |
