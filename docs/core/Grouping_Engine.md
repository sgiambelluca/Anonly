...
61. **Invariante de `heldManual` (ADR-175 §1)**: tras cualquier secuencia de fusión, división, reclasificación, eliminación, `dropOccurrences`, `restoreCheckpoint` y resolución, ningún conflicto tiene `resolved: true` con `heldManual`, y `heldManualOccurrences` tiene exactamente los ids de los conflictos `heldManual` sin resolver. Fusionar, dividir o reclasificar el grupo detectado deja el choque pendiente.
