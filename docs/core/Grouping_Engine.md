...
- [ ] 15t. (Hito 12.5 — ADR-172) `createCheckpoint`/`restoreCheckpoint`/`discardCheckpoints` con copia estructural de la sesión (misma función de copia que `previewEdit`), límite `MAX_EDIT_CHECKPOINTS`, descarte en `reopenSession`/`closeSession`, restauración que emite solo la diferencia. Casos 51-53, cinco filas en §14.
