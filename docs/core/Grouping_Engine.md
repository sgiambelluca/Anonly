...
- [ ] 15u. (Hito 12.5 — ADR-173) Rechazos de `applyGroupMerge` (tipos distintos, mismo grupo) y `applyGroupSplit` (vacío, ajeno, todos) con `GroupingInvalidPatchError` sin mutar; `previewEdit` los hereda. Casos 54-55, tres filas en §14.
- [ ] 15v. (Hito 12.5 — ADR-174) Ocurrencia manual perdedora retenida en el conflicto (`heldManual`), resolución con `winner`, retención incluida en los puntos de restauración. Casos 56-58, cuatro filas en §14.
