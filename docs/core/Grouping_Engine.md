...
| `bounded similarity predicate equals full normalized comparator` | `unit.test.ts` | unit | caso 46, ADR-182: diferencial con semilla fija, umbrales en/afuera del borde, longitudes distintas, vacíos, Unicode UTF-16 y umbrales especiales |
| `common affix trimming preserves normalized comparator` | `unit.test.ts` | unit | caso 47, ADR-183: diferencial con afijos comunes y desiguales, distancia exacta, bordes flotantes, Unicode UTF-16 y ruta especial |
| `fuzzy matching preserves first eligible group and alias order` | `unit.test.ts` | unit | caso 46, ADR-182: cadena no transitiva y grupos con varios alias; comparar grupos, miembros y orden |
| `type correction fuzzy pass keeps the same destination` | `edge.test.ts` | edge | caso 46, ADR-182: los dos call sites comparten el predicado; no cambia ADR-085 |
| `distinct and repeated grouping curves keep fingerprints` | `tests/perf/grouping-worst-case.ts` | perf opt-in | caso 46, ADR-182: 250–2000 valores; tiempo por motor y lookup inclusivo, sin SLA absoluto como gate |
