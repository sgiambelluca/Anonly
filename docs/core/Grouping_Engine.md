...
export class GroupingEngine implements IEngine {
  ...
  // Sus registros se conservan solo para el dedup: sin grupo vivo, no ocupan lugar (ADR-177 §1).
  ...
  liftRemoval(documentId: string, value: string): void;   // ADR-177 §2 (reemplaza ADR-176 §4): también borra todo registro NO VIVO con ese valor (grupo eliminado o SUPPRESSED_GROUP_ID)
  // ADR-176 §3: qué pasó con cada ocurrencia Manual procesada desde el último reopenSession.
  // Solo lo llama el façade (addManualEntity). Sesión inexistente -> listas vacías + warn.
  ...
  // ADR-172 §1: copia estructural de la sesión completa (grupos con campos
  // internos, registros de ocurrencias, reglas, conflictos, typeCorrections,
  // removedValues, contadores de nextIndex); hasta MAX_EDIT_CHECKPOINTS por sesión.
  createCheckpoint(documentId: string): string;
  // Reemplaza la sesión por la copia y emite la diferencia: ENTITY_GROUP_REMOVED /
...
| `CONFLICT_RESOLVE_REQUESTED` (canal `ui`) | usuario resuelve conflicto | `applyConflictResolve`. Con `winner` (ADR-174 §3) sobre un conflicto `heldManual`: `"manual"` agrupa la ocurrencia manual retenida por `groupOccurrence`, `"detected"` la descarta (identidad registrada) |
...
|---|---|---|---|---|
| `GROUPING_INVALID_PATCH` | `GroupingInvalidPatchError` | `GroupUpdateRequested.patch` contiene campos inmutables o inválidos; **fusión** de tipos distintos o de un grupo consigo mismo (ADR-173 §1); **división** con `occurrenceIds` vacío, ajeno al grupo o con todos sus members (ADR-173 §2); `winner` en un conflicto sin `heldManual` (ADR-174 §3) | no | rechazar el request, loguear warn |
...
48. **Eliminar una entidad (ADR-171 §2)**: `applyGroupRemove` quita el grupo y emite `ENTITY_GROUP_REMOVED`; los conflictos de ese grupo se descartan con `CONFLICT_RESOLVED` (como en el caso 25); su `indexInType` queda como hueco (caso 15) hasta la próxima renumeración de `finishSession`; cada alias normalizado entra a `Session.removedValues`; los registros de ocurrencias se conservan **solo** para el dedup: sin grupo vivo, no participan en la contención ni en la superposición (ADR-177 §1, casos 65-66). Grupo inexistente → `warn` + no-op. Pedirlo dos veces es idempotente.
...
54. **Fusión inválida (ADR-173 §1)**: `applyGroupMerge` con grupos de tipos distintos, o `sourceGroupId === targetGroupId` → `GroupingInvalidPatchError`, `warn`, sin eventos ni mutación. `previewEdit({ kind: "merge" })` del mismo caso → `InvalidInputError`.
55. **División inválida (ADR-173 §2)**: `applyGroupSplit` con `occurrenceIds` vacío, con un id que no es member, o con **todos** los members → `GroupingInvalidPatchError`, `warn`, sin eventos ni mutación: ningún grupo queda sin members ni se crea uno vacío. `previewEdit({ kind: "split" })` del mismo caso → `InvalidInputError`.
56. **Una ocurrencia manual que pierde queda retenida (ADR-174 §1)**: una ocurrencia `source: Manual` que pierde una superposición contra otra de otro tipo (empate de `confidence` contra `Regex`, o menor) **no se agrupa ni se descarta**: su identidad se registra, queda retenida en el conflicto y el conflicto sale con `heldManual: true`. Una ocurrencia manual que **gana** se agrupa como hoy, sin `heldManual`.
57. **Resolver con `winner` (ADR-174 §3)**: `"manual"` agrupa la retenida por `groupOccurrence` (se une a un grupo de su valor y tipo, o crea uno) sin tocar la detección existente, y resuelve con `resolvedType` = tipo manual; `"detected"` la descarta y resuelve con el tipo detectado; sin `winner` = `"detected"`; `winner` en un conflicto sin `heldManual` → `GroupingInvalidPatchError`.
58. **La retención sobrevive y no se duplica (ADR-174 §1, §3)**: tras `"detected"`, re-emitir la misma ocurrencia manual (re-aplicación de literales tras un re-análisis) la descarta por el dedup de identidad sin crear otro conflicto. Los puntos de restauración (ADR-172) incluyen las ocurrencias retenidas.
59. **Se oculta sola si la detección desaparece (ADR-175 §1)**: con un conflicto `heldManual` pendiente, `GROUP_REMOVE_REQUESTED` del grupo detectado → la retenida se agrupa por el camino de `winner: "manual"` (`CONFLICT_RESOLVED` con el tipo manual, sin `heldManual`); si su valor normalizado quedó en `removedValues` por esa eliminación, se quita. Lo mismo con `dropOccurrences({ source })` que deja sin members el grupo detectado y no alcanza a la retenida.
60. **`dropOccurrences` se lleva la retenida (ADR-175 §1)**: `dropOccurrences({ pageIndices })` sobre la página de la retenida → sale de `heldManualOccurrences` **sin** dejar identidad registrada; el conflicto queda `resolved: true` **sin** `heldManual`. Re-emitir la misma ocurrencia manual con la detección presente crea un conflicto `heldManual` **nuevo**; sin la detección, se agrupa.
61. **Invariante de `heldManual` (ADR-175 §1, ADR-176 §2)**: tras cualquier secuencia de **fusión, división**, reclasificación, eliminación, `dropOccurrences`, `restoreCheckpoint` y resolución: ningún conflicto tiene `resolved: true` con `heldManual`; `heldManualOccurrences` tiene exactamente los ids de los conflictos `heldManual` sin resolver; y **todo conflicto sin resolver apunta a un grupo que existe**. La fusión reapunta todo conflicto sin resolver de `source` a `target`; la división pasa un conflicto `heldManual` sin resolver al grupo (original o nuevo) que se queda con el member que se superpone a la retenida (criterio de ADR-107). Tras fusionar el grupo detectado y eliminar el destino, la retenida se oculta sola (caso 59). **ADR-177:** la secuencia incluye una eliminación **seguida** de un agregado manual superpuesto a lo eliminado, que no puede dejar un conflicto sin resolver apuntando al grupo eliminado (caso 66).
62. **Un agregado nuevo reabre la decisión (ADR-175 §2)**: tras `winner: "detected"`, `liftRemoval(documentId, value)` con el mismo valor normalizado olvida la identidad suprimida; re-emitir la ocurrencia manual crea un conflicto `heldManual` nuevo. Sin `liftRemoval` (re-aplicación de literales), el caso 58 no cambia. El rechazo de `winner` sobre un conflicto sin `heldManual` se loguea **una** vez (hallazgo de la revisión 2).
63. **`manualOutcome` (ADR-176 §3)**: desde cada `reopenSession`, Grouping anota por cada ocurrencia `source: Manual` qué registro la absorbió —el suyo si se agrupó; el existente si el dedup la descartó; el contenedor si quedó contenida (ADR-117); el conflicto si quedó retenida o si el dedup la descartó contra una retenida pendiente; nada si fue suprimida—. `manualOutcome` devuelve los grupos **actuales** de esos registros sin repetir y los conflictos que siguen sin resolver con `heldManual`. Contenida: agregar `4567-8901` como Teléfono dentro del Teléfono detectado «11 4567-8901» → `groupIds` con ese grupo. Grupo reclasificado (ADR-085) → `groupIds` con el grupo aunque su tipo ya no sea el pedido. La anotación se vacía en `reopenSession` y `closeSession`, no sale en el snapshot ni entra en los puntos de restauración.
64. **`liftRemoval` olvida todo registro no vivo del valor (ADR-176 §4, reemplazado por ADR-177 §2)**: tras eliminar un grupo (ADR-171), `liftRemoval` + re-emitir una ocurrencia manual en la **misma** posición (mismo `pageIndex`, `bbox`, tipo y valor) que un member eliminado → se agrupa en un grupo **nuevo**; no la descarta el dedup contra el registro del grupo eliminado. Los registros vivos con ese valor no se tocan. El test tiene que usar la misma posición: con otra posición el caso no se ejerce (hallazgo del planificador en la revisión 4, B4-0).
65. **Una entidad eliminada no contiene (ADR-177 §1)**: se detecta la Persona «Juan Perez», se elimina, y después llega «Perez» como Persona (`Manual` o detección) contenido en esa posición → se agrupa (grupo nuevo). `manualOutcome` devuelve ese grupo; el invariante de `Contracts.md` §3.5 se cumple. Con el contenedor **vivo** el caso 72 no cambia.
66. **Una entidad eliminada no choca (ADR-177 §1)**: se detecta un Email, se elimina, y después llega «email juan@x.com.» como Persona superpuesto (ratio > 0.5) → se agrupa, **sin** `CONFLICT_DETECTED` y sin conflicto `heldManual`. Lo mismo contra un registro suprimido (`SUPPRESSED_GROUP_ID`). El dedup por identidad sigue mirando los registros no vivos: re-emitir la ocurrencia **exacta** eliminada sin `liftRemoval` se descarta (casos 49 y 58 sin cambios).
67. **Lo contenido se oculta solo si su contenedor se elimina (ADR-178 §1-§2, B5-1 de la revisión 5)**: se detecta «Juan Perez» (x=100); se agrega a mano «Perez» con una aparición adentro (x=160) y otra suelta (x=400) → la de adentro queda guardada a nombre del contenedor, sin registro ni grupo, y `manualOutcome` no cambia. Tras `applyGroupRemove` de «Juan Perez», la de x=160 se re-procesa y entra al grupo «Perez» (members 160 y 400) en la misma operación, con `ENTITY_GROUP_UPDATED`. **El orden no cambia el resultado**: eliminar primero y agregar después da los mismos members, y un re-análisis posterior (`dropOccurrences({ pageIndices })` + re-detección + re-aplicación del literal sin `liftRemoval`) no cambia qué se oculta. Si lo re-procesado cae en otro contenedor vivo, queda guardado a nombre de ese; si choca, queda retenido (ADR-174). Una ocurrencia contenida que **no** es manual se sigue descartando sin guardar. Re-emitir la misma ocurrencia manual contenida no la guarda dos veces.
68. **Vida de lo guardado (ADR-178 §2-§3)**: `dropOccurrences` que borra el registro contenedor **y** alcanza a la ocurrencia guardada (misma página, o misma fuente) → se descarta con él; si el filtro **no** la alcanza → se re-procesa. Fusión, división y reclasificación no la tocan. `restoreCheckpoint` a un punto anterior a la eliminación la devuelve guardada a su contenedor, y rehacer la vuelve a agrupar. Sobrevive a `reopenSession`, muere en `closeSession` y no sale en el snapshot.
69. **Eliminar puede abrir un choque (ADR-178 §2, O6-1 de la revisión 6)**: se detectan la Persona «Juan Perez» y la Organización «Perez SA», superpuestas en «Perez»; se agrega a mano «Perez» como Persona, contenido en «Juan Perez» (queda guardado). Tras `applyGroupRemove` de «Juan Perez», el re-proceso choca contra «Perez SA» y pierde: queda **retenido** con `CONFLICT_DETECTED` y `heldManual: true` (ADR-174 §1), apuntando al grupo de «Perez SA», que existe (caso 61). No se agrupa ni se pierde. `restoreCheckpoint` a antes de la eliminación retira el conflicto y devuelve lo guardado a su contenedor. `winner: "manual"` lo agrupa como en el caso 57.
...
70. **Distancia acotada sin cambiar grupos (ADR-182)**: para umbral finito `0 < t < 1` y strings no vacíos, calcular `k = min(L, ceil((1-t)*L)+1)` con `L = max(lengths)` en UTF-16. Rechazar si la diferencia de longitudes supera `k`; evaluar solo la banda `|i-j| <= k` y cortar si el mínimo de una fila supera `k`. Si la distancia final está dentro de la banda, decidir con **la misma** comparación `1 - distance/L >= t`. En umbrales especiales o vacíos se usa la ruta completa. Un rechazo rápido nunca cambia el primer grupo/alias elegible, ni el pase exacto, ni el guard de tipos de ADR-073, ni la corrección de tipo de ADR-085. Una cadena A~B/B~C/A!~C conserva el orden de primera coincidencia. No se introduce límite de candidatos ni timeout.

71. **Afijos comunes sin cambiar la distancia (ADR-183)**: en la ruta acotada del caso 70, calcular `L` y `k` con los strings originales, recortar el máximo prefijo común y después el máximo sufijo común de los restos en unidades UTF-16. Aplicar la DP/banda a esos restos; si uno quedó vacío, la distancia es la longitud del otro. Decidir con `1 - distance/L >= t`, usando el `L` **original**. El mismo par debe dar el mismo booleano que `levenshteinNormalized(a,b) >= t`, incluidos afijos desiguales y bordes del umbral. No cambia ningún grupo, alias, miembro, evento u orden.
...
|---|---|---|---|
| `applyGroupMerge rejects different types and self-merge without mutating` | `edge.test.ts` | edge | caso 54 (ADR-173 §1) |
| `applyGroupSplit rejects empty, foreign and all-members occurrenceIds without mutating` | `edge.test.ts` | edge | caso 55 (ADR-173 §2) — **incluye el invariante**: ningún grupo queda sin members |
| `previewEdit rejects the invalid merges and splits the real request rejects` | `edge.test.ts` | edge | casos 54-55 + ADR-170 §2 (reemplaza al que solo probaba grupo inexistente) |
| `a losing manual occurrence is held, not grouped, and marks the conflict heldManual` | `unit.test.ts` | unit | caso 56 (ADR-174 §1) |
| `resolve winner manual groups the held occurrence; detected discards it` | `unit.test.ts` | unit | caso 57 (ADR-174 §3) |
| `winner on a conflict without heldManual is rejected` | `edge.test.ts` | edge | caso 57 |
| `re-emitting a discarded held manual occurrence does not recreate the conflict` | `edge.test.ts` | edge | caso 58 |
| `restoring a checkpoint brings back the held manual occurrence and winner manual groups it` | `unit.test.ts` | unit | caso 58, segunda cláusula |
| `removing the detected group of a held conflict groups the held occurrence` | `unit.test.ts` | unit | caso 59 (ADR-175 §1) |
| `dropOccurrences by source that empties the detected group groups the held occurrence` | `edge.test.ts` | edge | caso 59 |
| `dropOccurrences by page takes the held occurrence and closes the conflict without heldManual` | `edge.test.ts` | edge | caso 60 |
| `no path leaves a resolved conflict with heldManual` | `edge.test.ts` | edge | caso 61 — recorre las operaciones del caso y afirma el invariante tras cada una |
| `liftRemoval reopens a choice resolved as detected` | `unit.test.ts` | unit | caso 62 (ADR-175 §2) |
| `merge and split keep a pending conflict pointing to an existing group` | `edge.test.ts` | edge | caso 61 (ADR-176 §2) — el test del invariante suma fusión y división |
| `removing the merge target of a held conflict groups the held occurrence` | `edge.test.ts` | edge | caso 61 |
| `manualOutcome reports grouped, deduped, contained and held occurrences` | `unit.test.ts` | unit | caso 63 (ADR-176 §3) |
| `manualOutcome is cleared on reopenSession and absent from the snapshot` | `unit.test.ts` | unit | caso 63 (errata N4-1 de la revisión 4: vive en `unit.test.ts`, no en `edge.test.ts`) |
| `liftRemoval forgets removal-suppressed identities of the value` | `unit.test.ts` | unit | caso 64 (ADR-176 §4) |
| `re-adding a removed value at the same position creates a new group` | `unit.test.ts` | unit | caso 64 (ADR-177 §2) — **misma** posición que el member eliminado; los registros vivos del valor no se tocan |
| `an occurrence inside a removed entity is grouped, not discarded as contained` | `edge.test.ts` | edge | caso 65 (ADR-177 §1) — manual (con `manualOutcome`) y detección |
| `an occurrence overlapping a removed or suppressed record raises no conflict` | `edge.test.ts` | edge | caso 66 (ADR-177 §1) |
| `identity dedup still sees removed records without liftRemoval` | `edge.test.ts` | edge | caso 66 — el dedup no cambia |
| `a manual occurrence contained in a removed entity is grouped on removal` | `unit.test.ts` | unit | caso 67 (ADR-178 §2) — el probe de B5-1 |
| `removal order does not change what is hidden` | `edge.test.ts` | edge | caso 67 (ADR-178 §4) — los dos órdenes y el re-análisis posterior dan los mismos members |
| `a detected contained occurrence is not kept` | `edge.test.ts` | edge | caso 67 — solo se guarda lo manual, y sin duplicar |
| `kept contained occurrences follow dropOccurrences and checkpoints` | `edge.test.ts` | edge | caso 68 (ADR-178 §2-§3) |
| `removing a container can hold a kept occurrence in a new conflict` | `edge.test.ts` | edge | caso 69 (O6-1) — el invariante del caso 61 se cumple tras la eliminación |
| **`restoreCheckpoint returns the exact session, including internal state`** | `contract.test.ts` | contract | caso 51 (ADR-172) — **el test de que deshacer no miente**: una secuencia por cada edición que §3.3b declaraba sin inversa (fusión, división, reclasificación, eliminación) |
...
| `an occurrence fully inside another of the same type is not recorded` | `edge.test.ts` | edge | caso 72 (ADR-117) — el caso medido: el apellido arrancado de adentro de tres nombres completos |
| `a PARTIAL overlap of the same type is still recorded` | `edge.test.ts` | edge | caso 72 (ADR-117) — el límite de la regla estricta |
| `containment of a DIFFERENT type still goes through the overlap conflict` | `edge.test.ts` | edge | caso 72 (ADR-117) — los casos 7-8 no se tocan |
| `does not treat a neighbour inside the ENVELOPE of a multi-line entity as contained` | `edge.test.ts` | edge | caso 72 (ADR-117) — ADR-107 aplicado a la contención |
...
| **`"Diego Ramos" and "Diego Ram0s" still group together`** | `unit.test.ts` | unit | caso 4 — **no-regresión del texto libre**: si esto se cae, ADR-073 rompió lo que vino a proteger. Ídem `Organization` y `Address` |
...
- [x] 15y. (Hito 12.5 — ADR-177) Solo un registro vivo (`session.groups.has(rec.groupId)`) participa en `findContainingRecord` y `findOverlapConflict`; el dedup por identidad no cambia; `liftRemoval` borra todo registro no vivo del valor. Casos 24, 48 y 61 ampliados, 64 corregido, 65-66 nuevos, cuatro filas en §14 y el test del caso 61 cruza una eliminación con un agregado posterior superpuesto.
...
`FUZZY_MATCHING_TYPES = { Person, Organization, Address }` es una **lista cerrada y explícita**, no una regla derivada (ADR-073 §1): una condición del estilo "los que no tienen `checksum`" volvería a atar el comportamiento a una propiedad que puede cambiar por otra razón, y movería un tipo de un lado al otro en silencio. Son los tres tipos que emite NER, y los únicos donde una diferencia de un carácter puede ser ruido del canal —un OCR que lee `"Diego Ram0s"`— en vez de dato. `Custom` queda **afuera**: su valor lo define un patrón del usuario y el motor no tiene base para decidir si un carácter de diferencia es un typo o un identificador distinto.
