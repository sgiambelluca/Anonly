...
export class GroupingEngine implements IEngine {
  ...
  liftRemoval(documentId: string, value: string): void;   // ADR-176 §4: también olvida toda identidad suprimida con ese valor (eliminación y winner "detected")
...
64. **`liftRemoval` olvida toda supresión del valor (ADR-176 §4)**: tras eliminar un grupo (ADR-171), `liftRemoval` + re-emitir una ocurrencia manual en la **misma** posición que un member eliminado → se agrupa (no la descarta el dedup contra la identidad suprimida).
...
| `manualOutcome is cleared on reopenSession and absent from the snapshot` | `edge.test.ts` | edge | caso 63 |
