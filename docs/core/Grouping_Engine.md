...
- **Corre en main thread** (no en Worker). En R1/R2 medidos fue ligero, pero 2.000 valores distintos de 36 caracteres bloquearon 14,53 s antes de ADR-176; < 5% del pipeline no es una garantía.
- Costo: depende del número de grupos/alias candidatos y de la longitud de valores. El pase difuso usa distancia acotada con banda y corte temprano para rechazos imposibles (ADR-176), y recorta afijos comunes antes de la DP (ADR-177), sin cambiar la primera coincidencia.
...
| `bounded similarity predicate equals full normalized comparator` | `unit.test.ts` | unit | caso 46, ADR-176: diferencial con semilla fija, umbrales en/afuera del borde, longitudes distintas, vacíos, Unicode UTF-16 y umbrales especiales |
| `common affix trimming preserves normalized comparator` | `unit.test.ts` | unit | caso 47, ADR-177: diferencial con afijos comunes y desiguales, distancia exacta, bordes flotantes, Unicode UTF-16 y ruta especial |
| `fuzzy matching preserves first eligible group and alias order` | `unit.test.ts` | unit | caso 46, ADR-176: cadena no transitiva y grupos con varios alias; comparar grupos, miembros y orden |
| `type correction fuzzy pass keeps the same destination` | `edge.test.ts` | edge | caso 46, ADR-176: los dos call sites comparten el predicado; no cambia ADR-085 |
| `distinct and repeated grouping curves keep fingerprints` | `tests/perf/grouping-worst-case.ts` | perf opt-in | caso 46, ADR-176: 250–2000 valores; tiempo por motor y lookup inclusivo, sin SLA absoluto como gate |
...
- [x] 15r. (Hito 11, ADR-176) Agregar predicado de distancia acotada en `levenshtein.ts` sin alterar `levenshteinNormalized`; usarlo en los dos pases difusos de `grouping.engine.ts`. Preservar orden y primera coincidencia. Añadir diferencial del caso 46, tests de grupo/corrección y repetir el banco opt-in más R1/R2. Si la curva sigue bloqueando varios segundos, informar el límite antes de proponer un índice.
- [x] 15s. (Hito 11, ADR-177) Recortar prefijo/sufijo comunes dentro del predicado acotado, con radio y denominador originales; mantener rutas especiales y primera coincidencia. Añadir diferencial del caso 47 y repetir banco del motor más R1/R2, con huellas exactas y tiempo residual declarado.
