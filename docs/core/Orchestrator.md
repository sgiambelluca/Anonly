...
- `addManualEntity` y los choques (ADR-174 §2): después de `finishSession`, completa `ManualEntityResult.heldConflictIds` con los conflictos no resueltos con `heldManual` cuyo candidato `Manual` tiene el valor agregado (del snapshot de Grouping). `occurrenceCount > 0` con `heldConflictIds` no vacío **no** es un agregado exitoso: la UI abre el diálogo de choque en vez del toast de alta.
