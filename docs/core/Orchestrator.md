...
> **Nota (ADR-166, 2026-09-17 — el pool de NER también se da de baja al terminar su etapa)**: `runDetectionStage` invoca `this.engines.ner.releaseIdleWorkers()` (`NER_Engine.md` §6) en un `finally` que envuelve la etapa de detección, en los tres caminos terminales: éxito, cancelación y fallo. Es el **mismo mecanismo** de ADR-157 aplicado al otro motor, con la misma guarda: si el pool no está ocioso —un batch todavía en vuelo en una cancelación— no hace nada, sin lanzar ni enmascarar el error real de la etapa, y la memoria la libera el temporizador de ADR-080 como hasta hoy. **La razón, en cambio, no es la misma**: en OCR el minuto de inactividad transcurría durante la detección, que es donde está el pico, así que liberar bajaba ese pico. Acá el minuto transcurre **después** de `PIPELINE_READY`, con todo terminado: lo que baja no es el pico del documento actual sino el nivel sostenido mientras el usuario revisa, y con él el punto de partida del documento siguiente (ADR-154 §2 lever 3). T-7 midió **922 MB en P1 y 1027 MB en P2** liberándose 60 s tarde. `idleDisposeMs` no cambia: sigue en 60 s para los cinco pools, y esto actúa antes. El pool queda usable: un `reanalyze` que vuelva a detectar —`ner.enabled: true` (caso 18) o `ocr.languages` (caso 20)— lo reconstruye perezoso y paga la recarga del modelo, **942,94 ms medianos en frío**, como costo declarado (ADR-166 §2). Reemplaza lo que ADR-157 §4 había dejado abierto («NER no entra en este ADR»), ahora con los dos lados medidos. Ver §13 caso 35.
...
35. **Baja del pool de NER al terminar `runDetectionStage` (ADR-166)**: `this.engines.ner.releaseIdleWorkers()` corre en un `finally` que envuelve la etapa de detección. **Camino feliz**: los batches ya resolvieron, el pool está ocioso y la baja ocurre — el modelo se suelta antes de que la cascada llegue a `GROUPING_FINISHED`/`PIPELINE_READY`, porque Grouping no usa NER. **Cancelación o fallo a mitad de etapa**: la llamada puede encontrar el pool NO ocioso (un batch en vuelo); la guarda de `releaseIdleWorkers()` la vuelve un no-op, sin lanzar ni enmascarar el error o la cancelación real, y la memoria la libera el temporizador de ADR-080. **Ninguna de las dos ramas dispone el motor**: sigue usable, y los dos flujos de reanálisis que vuelven a detectar (casos 18 y 20) lo reconstruyen perezoso pagando la recarga. El precalentado de la página 1 (caso 32) y el seed del preview no se ven afectados: son de Render.
