...
- `addManualEntity` y los choques (ADR-174 §2, **ADR-175 §3**): mientras dura `findLiteral`, junta el `normalizedValue` de cada ocurrencia `source: Manual` que emite `ENTITY_FOUND` para ese documento. Después de `finishSession`, del snapshot de Grouping: `heldConflictIds` = conflictos no resueltos con `heldManual` cuyo candidato `Manual` cumple `normalizeEntityValue(candidate.value) ∈ conjunto`; `groupIds` = grupos con algún member que cumple `normalizeEntityValue(member.value) ∈ conjunto`. **Nunca** igualdad exacta con `request.value`: el documento trae la puntuación pegada (ADR-115). Invariante: `occurrenceCount > 0` ⇒ `heldConflictIds` o `groupIds` no vacío.
