...
- `addManualEntity` y los choques (ADR-174 §2, **ADR-175 §3**): mientras dura `findLiteral`, junta el `normalizedValue` de cada ocurrencia `source: Manual` que emite `ENTITY_FOUND` para ese documento. Después de `finishSession`, del snapshot de Grouping: `heldConflictIds` = conflictos no resueltos con `heldManual` cuyo candidato `Manual` es del `entityType` pedido y cumple `normalizeEntityValue(candidate.value) ∈ conjunto`; `groupIds` = grupos con (a) un member cuyo `occurrenceId` es de una ocurrencia emitida, o (b) `group.type === entityType` pedido y un member con `normalizeEntityValue(member.value) ∈ conjunto` (junta también el `id` de las emitidas). Un grupo de **otro** tipo sobre el mismo texto no cuenta (errata 2026-09-24 de ADR-175 §3: sin la condición de tipo, el caso 43 se contradecía). **Nunca** igualdad exacta con `request.value`: el documento trae la puntuación pegada (ADR-115). Invariante: `occurrenceCount > 0` ⇒ `heldConflictIds` o `groupIds` no vacío.
...
- [ ] 32. (Hito 12.5 — ADR-175 §3) `heldConflictIds` y `groupIds` por el `normalizedValue` de las ocurrencias emitidas en el agregado. Casos 43-44.
