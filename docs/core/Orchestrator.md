...
- [ ] 24. (Hito 10.7, PR 3 — ADR-061 §4/§5/§6/§8; v1.7.0) Las cuatro entradas nuevas de `IPipelineOrchestrator`: `addManualEntity` orquestando `reopenSession` → `regex.findLiteral` → `finishSession`; `findText` sobre la misma búsqueda literal, devolviendo `TextMatch[]` en vez de emitir; `getPageWords`/`getPageSize` por página y a demanda, con `InvalidInputError` sobre `documentId`/`pageIndex` inexistente. **Estado retenido nuevo**: la lista de `{ value, entityType }` por documento, **re-aplicada después de toda re-detección** y descartada en `closeDocument`/`dispose`. El test de la re-aplicación tras `reanalyze` (§14) es el que protege el modo de falla silencioso de ADR-061 §5 — sin él el dato desaparece del árbol sin aviso y se exporta sin anonimizar.
