...
**Estado de implementación**: Hito 4, checklist §15 (items 1-18) implementado; item 19 (propagación de `bbox.rotation`) en Hito 10.8. `findLiteral` (item 10b) y `searchText` (item 10c) implementados en los PRs 2 y 3c del Hito 10.7 respectivamente. Pendiente: `cancel.test.ts`/`perf.test.ts` de §14 en Hito 11 (ver nota al final de §15).
