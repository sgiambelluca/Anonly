...
> **Nota (v1.14.0, ADR-175, 2026-09-24 — email default lineal)**: el patrón `email` conserva exactamente el lenguaje, spans, orden, normalización y salida de `\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}\b` con flag global, pero su recorrido productivo se especializa mediante un escáner lineal por `@`. El `RegExp` queda como oráculo acotado en tests. Se especializa el objeto del **default**, no el id: un custom llamado `email` sigue siendo custom. El hallazgo de 42,5 s sobre 160 KiB sin `@`, los controles R1/R2 y el criterio diferencial están en `roadmap/Patron_Email_Regex_Medicion.md` y ADR-175. No cambia ningún contrato público ni la cancelación entre páginas.

> **Nota (v1.13.0, ADR-122, 2026-09-02 — una carátula se escribe en mayúsculas)**: `caratula-ar` exigía `\p{Lu}\p{Ll}+` de los dos lados y un `s/`/`c/` en minúscula. Una carátula de expediente se escribe `APELLIDO, NOMBRE SEGUNDO S/ RECURSO DE CASACIÓN`: medido sobre un fallo escaneado de 20 páginas, **0 matches** — el patrón estaba apagado justo sobre el formato más previsible que hay, que es para el que ADR-092 lo escribió. Tres bloqueos a la vez, y hasta arreglar los tres sigue en cero: la **caja**, la **cantidad de nombres de pila** (uno solo) y el **separador**. Ahora los dos lados aceptan `\p{Lu}\p{L}+`, el separador acepta `[CS]\/` y —solo en la rama que **cierra** con `s/`/`c/`— el tramo de nombres se abre a `{1,3}`. La rama anclada por MARCA (ADR-103) queda **igual**: una palabra de cada lado, en Título. La distinción es el punto: la marca ancla por la izquierda, el `s/` ancla por la **derecha**, y solo el segundo acota lo que el match se traga — que es la razón real del límite de ADR-092 §1, no un capricho. El **orden de las ramas importa**: la alternancia de JS es ordenada, así que la rama con cierre va **primero** o el match corto trunca el nombre (`"Autos: López, María Fernanda c/ Empresa"` → `"López, María"`). El `8/` y el `$/` son los dos glifos que Tesseract puso de verdad en lugar de la `S`, en 2 de las 20 páginas: sin ellos el patrón queda en 18/20. Medido: **0/20 → 20/20**, un hallazgo extra que **no es un falso positivo** sino una carátula real que antes se perdía (`"Alberti, Marta Beatriz c/ Provincia de Corrientes"`, con dos nombres), las cinco trampas de ADR-092 y ADR-103 siguen sin matchear, y `pnpm test:quality` queda idéntico (61/61, 61/62). Sobre el escaneo, de las 40 vecinas que tocan una carátula **0 son de otro tipo** — el riesgo que ADR-092 midió no aparece; sí hay **35 ocurrencias de NER del mismo tipo contenidas enteras**, que ADR-117 no registra: no se pierde cobertura (la carátula es más ancha) pero los grupos se reparten distinto. `entityType`, `checksum`, `normalizer` y `maskFormat` **no cambian**. Ver la tabla de patrones, §13 caso 31 y §14.

> **Nota (v1.12.0, ADR-115, 2026-09-02 — la puntuación pegada no es parte del valor)**: `buildManualOccurrence` sacaba `normalizedValue` de `normalizeForComparison`, que **no recorta los bordes**. Como un match manual entra con las palabras enteras (ADR-089 §3) y `Page.words` separa por whitespace (ADR-020 §1), el mismo apellido llega con lo que tuviera pegado —`SUAREZ,` en un sello, `“SUAREZ,` en una carátula, `Suarez.` al final de una oración— y salía con **tres claves de agrupado distintas**. El pase difuso de `grouping-engine` no las junta: contra `suarez` dan **0,857 y 0,750**, por debajo del umbral de 0,88. Medido sobre un expediente escaneado, un solo apellido agregado a mano abría **cuatro grupos** en vez de uno. Pasa a usar `normalizeEntityValue` (`@anonly/shared`, ADR-115 §1): `normalizeForComparison` más el recorte de todo lo que no sea letra ni dígito en los **bordes**, por clase Unicode y no por una lista de signos —las comillas tipográficas de una carátula no estaban en ninguna lista—. La puntuación **interna** no se toca. `normalizeForComparison` no cambia: sigue siendo la de `findLiteral` y la del léxico de género. Ver §13 caso 30 y §14.
...
> **Nota (v1.11.0, ADR-103, 2026-08-27 — la carátula se reconoce por su contexto, no por su coma)**: `caratula-ar` buscaba `Palabra, Palabra` en **cualquier parte del texto**, y eso produce dos clases de falso positivo que el checksum no puede filtrar porque miran el segundo término: el **adverbio inicial** (`"Finalmente, Alejandro"`) y —la grave— la **enumeración** (`"Abril, Facundo"`), donde **los dos son nombres reales y el patrón los emite como una sola persona**. Encontradas por el humano leyendo el PDF exportado de una pericia real. El match pasa a exigir una **marca de carátula adyacente**: precedido de `caratulado`/`Autos`/`causa`/`Expediente`, o seguido de `c/`/`s/`. Medido: los tres casos legítimos se conservan y los cinco falsos desaparecen, y de yapa cierra el residuo `"Buenos Aires, Argentina"` que la v1.8.0 aceptaba. **Esto revisa el juicio de la v1.8.0**, que descartó el lookbehind con la regla "no cambies un falso positivo por un falso negativo": la regla es correcta cuando hay uno de cada lado, pero un FP que **fusiona dos personas** no es de la misma moneda que uno que se ve y se apaga — el primero es una fuga silenciosa. El costo aceptado es que un `Apellido, Nombre` sin marca cerca (`"el Doctor Pérez, Juan"`) deja de detectarse por este patrón; en el cuerpo lo cubre NER. `checksum`, `normalizer`, `entityType` y `maskFormat` **no cambian**. Ver la tabla de patrones, §13 caso 31 y §14.
...
- **Se compara la corrida entera, no el carácter vecino**: lo que delata a `"00-000000"` como tramo de otra cosa está tres saltos a la izquierda (`PP`).
...
- **Corre en main thread** (no en Worker). En documentos normales medidos es ligero; un patrón adverso puede bloquearlo, de modo que el porcentaje del pipeline no es una garantía.
- El email default usa el escáner de ADR-175, con costo O(caracteres + matches) en vez del backtracking cuadrático de su `RegExp` de referencia. Los demás defaults y los custom conservan su ruta de ejecución.
...
- La especialización de email no añade timeout ni un chequeo intra-página de `abortSignal`; la cancelación de `process` continúa entre páginas. El escáner debe conservar exactamente los spans y el orden global que produciría la expresión de referencia, incluida la `@` tardía y el caso de dos matches separados por un tramo continuo de caracteres permitidos.
...
18. **Coincidencias solapadas**: buscar `"eta eta"` sobre `"eta eta eta"` emite **una** ocurrencia, no dos. Tras un match el barrido avanza el largo completo de la secuencia; los solapamientos no se reportan. Buscar `"eta"` sobre lo mismo sí emite tres — son matches disjuntos.
...
28. **Un tramo de un número de expediente no es un teléfono** (ADR-075 §2): sobre `"PP-13-00-000000-24/00"` no se emite ninguna ocurrencia — el match `"00-000000"` no tiene letras y su corrida sí (`PP`). Los casos que **sí** siguen emitiendo, y que son la mitad que protege contra la fuga: `"Tel: 0221-4567890."` (puntuación de oración alrededor), `"Tel:0221-4567890"` (la corrida corta en el `:`), `"0221-4567890,0221-4567891"` (dos ocurrencias, la coma no extiende) y `"34.567.891/2024"` (corrida sin letras ⇒ la guarda no aplica). El único que se pierde es `"Tel.0221-4567890"` (§6, residuo aceptado). **Corrección (2026-08-18, hallazgo de la revisión del Hito 10.9)**: este caso y las dos filas de §14 citaban `"4567-8900"`/`"4567-8901"`, que no matchean ningún patrón de teléfono real (verificado); ver la nota de ADR-075, Decisión §5.
30. **El mismo valor con distinta puntuación pegada agrupa junto** (ADR-115 §1): `normalizedValue` sale de `normalizeEntityValue`, que recorta los **bordes** no alfanuméricos, así que `“SUAREZ,`, `SUAREZ` y `Suarez.` dan la misma clave (`suarez`) y caen en un solo grupo. Los `value` conservan lo impreso — es lo que la UI muestra. La puntuación **interna** no se toca: `A.C.` → `a.c`, un CUIT con guiones queda igual. **Divergencia abierta**: `ner-engine` normaliza con su propia función, que recorta bordes pero **no saca diacríticos**, así que `maría` (NER) y `maria` (manual) todavía se parten — ver ADR-115, "Lo que este ADR NO cierra".
...
30. **La comparación literal es por sub-token, y se afloja distinto de cada lado** (ADR-089): `"Juan Pérez"` encuentra `"Juan" · "Pérez,Juan"` (puntuación interna) y `"20-12345678"` encuentra `"20-12345678-9"` (prefijo de identificador) en las **dos** entradas, con `bbox` de la palabra entera. `searchText("Eta")` encuentra además `"Anabella"`; `findLiteral` con `"Eta"` **no** la toca. **No** matchean, deliberadamente: `"Juan Pérez"` contra `"Pérez," · "Juan"` (orden invertido — es §23c, un problema de detección) y `"J. Pérez"` contra `"José Pérez"` (ADR-061 §2: no es búsqueda difusa). `"O'Brien"` se sigue encontrando por su propio texto: los dos lados se parten igual.
31. **Una carátula judicial es una entidad, un topónimo no** (ADR-092): `"Pérez, Juan"`, `"López, María Fernanda"` y `"Echeverria, Marta"` emiten `Person` con `confidence: 1.0`; `"San Miguel, Tucumán"`, `"Mar del Plata, Buenos Aires"`, `"La Plata, Buenos Aires"` y `"Código Civil, Título III"` **no** emiten. **Dos compuertas, las dos obligatorias** (ADR-092 + ADR-103): que el nombre de pila esté en `GENDER_LEXICON`, y —desde ADR-103— que haya una **marca de carátula adyacente**, sea la palabra que la introduce antes (`caratulado`/`Autos`/`causa`/`Expediente`, más las marcas de firma y de perito) o las partículas que separan a las partes después (`c/`, `s/`). La segunda es la principal: sin ella `Palabra, Palabra` aparece en prosa normal todo el tiempo, y el checksum no puede filtrarla porque mira el segundo término, donde sí hay un nombre. `"El actor, Juan Pérez"` tampoco emite, y no por la compuerta: ya está en el orden correcto y no es una carátula. **El apellido es una palabra**: sin la coma como ancla un apellido compuesto no se distingue de un topónimo, y de `"Ríos de Paz Alberti, Marta"` entra solo la última. **Los nombres de pila son varios o uno según si la carátula CIERRA** (ADR-122 §2): con `s/`/`c/` detrás el tramo llega a 3, sin cierre queda en 1. El límite de uno solo venía de que nada frenaba al cuantificador por la derecha — medido sobre la firma de la pericia, `"Echeverria, Marta Date"` cruzaba al run vecino y hacía **desaparecer** el grupo de Fecha por solapamiento (el mecanismo de ADR-088 §1); donde hay `s/` ese freno existe y es explícito, y ahí el límite deja de hacer falta. El costo aceptado sobrevive solo sin cierre: `"perito a López, María Fernanda,"` produce `"López, María"`. **La rama con cierre va primero en la alternancia**, o el match corto de la rama de marca trunca el nombre. **Caja alta** (ADR-122 §1): los dos lados aceptan `\p{Lu}\p{L}+` y el separador puede venir en alta, incluidos el `8/` y el `$/` que el OCR devuelve en lugar de `S/`. Sin eso, `"SUAREZ, BARTOLOME ARTURO S/ RECURSO"` daba **0 matches en 20 páginas**. **Residuo cerrado por ADR-103**: `"Buenos Aires, Argentina"` **ya no** emite — el checksum lo dejaba pasar porque "Argentina" es un nombre permitido del registro, pero sin marca de carátula cerca el match no ocurre. La v1.8.0 lo aceptaba con el criterio "no cambies un falso positivo por un falso negativo"; ADR-103 revisó ese juicio, porque la clase de falso positivo resultó mucho mayor (adverbio inicial, y sobre todo la **enumeración** `"Abril, Facundo"`, que **fusiona dos personas reales en una sola** — eso no es un destildado, es una fuga silenciosa). **El costo aceptado ahora es el inverso**: un `Apellido, Nombre` sin marca cerca (`"el Doctor Pérez, Juan"`) deja de detectarse por este patrón; en el cuerpo lo cubre NER.
32. **La característica telefónica no siempre tiene dos dígitos** (ADR-093): `"11 4567-8900"` (CABA, característica de 2), `"221 456-7890"` (La Plata, 3), `"341 456-7890"` (Rosario, 3), `"351 456-7890"` (Córdoba, 3) y `"2954 12-3456"` (Santa Rosa, 4) emiten `Phone`, con y sin el prefijo `+54`; `"+54 9 11 4567-8900"` también (el `9` del formato de móvil internacional). Antes de este ADR solo detectaba el primero. **No** matchean, deliberadamente, formas cuyo total de dígitos contiguos (separados solo por espacio o guion) no suma exactamente 10 por rama: tres grupos de una tarjeta (`"4532 1234 5678"`), tres de un IBAN (`"1234 5678 9012"`), dos números sueltos adyacentes (`"expediente 1234 5678"`) y el tramo de un expediente (`"PP-13-00-000000-24/00"`, caso 28) siguen sin emitir Phone — es la propiedad que distingue la alternancia enumerada de un rango laxo (ADR-093 §1). `phone-landline-ar` no se toca: `"0221-4567890"` lo sigue tomando ese patrón, no el móvil. El falso positivo preexistente del CUIT (caso 29 y ADR-093 §2) sigue produciendo el mismo valor, `"20-12345678"` — este ADR no lo introduce ni lo mueve.
...
| `default email scanner matches reference spans and values` | `unit.test.ts` | unit | caso 34, ADR-175: diferencial de lista ordenada en corpus generado con semilla fija y casos manuales acotados; regex anterior solo como oráculo del test |
| `default email scanner keeps events, normalization and bbox` | `contract.test.ts` | contract | caso 34, ADR-175: misma salida pública, incluidos orden y overlaps |
| `custom pattern named email uses custom path` | `edge.test.ts` | edge | caso 34, ADR-175: especialización por identidad del default; custom mantiene presupuesto y semántica |
| `long no-at and late-at text use linear email scan` | `tests/perf/regex-worst-case.ts` | perf opt-in | caso 34, ADR-175: curva de 2–160 KiB y cancelación; no gate temporal absoluto |
...
| `findLiteral does not report overlapping matches` | `unit.test.ts` | unit | caso 18: `"eta eta"` sobre `"eta eta eta"` emite una, no dos |
...
| `every occurrence of the same word shares normalizedValue, whatever punctuation is glued on` | `unit.test.ts` | unit | ADR-115 §1, caso 30 — falla si el call site vuelve a `normalizeForComparison` |
...
| **`"PP-13-00-000000-24/00" emits no Phone occurrence`** | `unit.test.ts` | unit | caso 28 (ADR-075 §2) — la cadena literal de la pericia |
...
| `an all-caps caption from a scanned header is detected` | `unit.test.ts` | unit | caso 31 — ADR-122 §1: el formato del sello, que daba 0/20 |
| `several given names are captured only when a s/ or c/ closes the caption` | `unit.test.ts` | unit | caso 31 — ADR-122 §2: los dos casos van juntos porque es el MISMO nombre y lo único que cambia es el cierre. Revertir el orden de las ramas lo rompe |
| `tolerates the two glyphs the OCR puts in place of the S` | `unit.test.ts` | unit | caso 31 — ADR-122 §3: `8/` y `$/`, sin ellos 18/20 |
| `an all-caps caption normalizes to the same key as the body name` | `unit.test.ts` | unit | caso 31 — ADR-122 §2: la inversión toma TODOS los nombres, o la carátula y el cuerpo caen en dos grupos |
| **`"Buenos Aires, Argentina" ya no emite: no hay marca de carátula cerca`** | `edge.test.ts` | edge | caso 31 — el residuo que ADR-103 cerró, fijado en un test para que no vuelva a abrirse |
...
- [x] 22. (Hito 11, ADR-175) Especializar solo el objeto del email default con el escáner lineal por `@` descrito en el ADR. Conservar la expresión como oráculo de tests, la ruta `runPattern`/presupuesto de todo custom, el `RawMatch` y todos los filtros/eventos posteriores. Añadir diferencial exacto de spans/valores, contrato de salida y casos límite de §14; repetir el banco opt-in y R1/R2 reales antes de cerrar el punto.
