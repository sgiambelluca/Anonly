...
29. **Un teléfono precedido por una palabra y un espacio sigue emitiendo** (v1.6.1, hallazgo post-mergeo): sobre `"CUIT 20-12345678-9"`, `phone-mobile-ar` matchea `" 20-12345678"` con el espacio adentro (su `[\s-]?` inicial es opcional y ese espacio cae ahí). La guarda de corrida mide sobre el match **recortado** de ese espacio de borde — nunca sobre el crudo — así que la corrida no se extiende hasta `"CUIT"` y el teléfono se emite. Es el caso real de `text-10p.pdf` (escenario 8 de E2E).
