...
| `PIPELINE_READY` | Orchestrator | UI | `{ documentId, groupCount, conflictCount }` | async | sí | none | Cuando llega a `Ready`. Puede emitirse **más de una vez** por documento: una por cada pasada de `reanalyze` (ADR-038 §5). |
...
| `PIPELINE_FAILED` | Orchestrator | UI | `{ documentId, error: SerializedEngineError }` | async | sí | none | Error fatal no recuperable. (Errata ADR-036 §9: decía `EngineError`; los payloads del bus transportan la forma serializada, `Contracts.md` §8.) |
...
| Evento | Emisor | Receptores | Payload | Timing | Idempotente | Orden | Notas |
|---|---|---|---|---|---|---|---|
| `ENTITY_FOUND` | Regex Engine, NER Engine | Grouping Engine | `{ documentId, occurrence: Occurrence }` | async | sí | por-página | `occurrence.source ∈ {regex, ner}`. Idempotencia real: Grouping descarta en silencio una ocurrencia cuya identidad `(entityType, pageIndex, bbox, normalizedValue)` ya está registrada en la sesión — invariante permanente que sostiene el re-análisis parcial (ADR-038 §3), no solo un caso especial de reanalyze. |
...
| Evento | Emisor | Receptores | Payload | Timing | Idempotente | Orden | Notas |
|---|---|---|---|---|---|---|---|
| `PREVIEW_UPDATED` | Render Engine | UI, Orchestrator | `{ documentId, pageIndex, kind: "original" \| "anonymized", canvasBlobUrl, degraded? }` | async | sí | por-página | Vista previa incremental de una página. `kind` indica a qué visor (original o anonimizado) corresponde el blob (ver ADR-016). El `canvasBlobUrl` lo crea el lado host del motor; el Orchestrator lo escucha solo para revocar el URL anterior de la misma clave (ADR-034 §5). `degraded` (ADR-062 §1) transporta las anotaciones `Degraded` de **este** render: **ausente ≡ vacío** (§2), el consumidor **reemplaza** el veredicto de la página en vez de acumularlo y **descarta los eventos con `kind: "original"`** (§3). Lo emite también el **cache hit**, desde la entrada guardada (§4) — si no, la marca de la UI parpadea al volver a una página ya vista. |
...
| `EXPORT_FAILED` | Export Engine | UI, Orchestrator | `{ documentId, error: SerializedEngineError }` | async | sí | none | El Orchestrator lo escucha para emitir `PIPELINE_FAILED` tras agotar reintentos (ver `06_Pipeline.md` §13). (Errata ADR-036 §9: decía `EngineError`.) |
...
| Evento | Emisor | Receptores | Payload | Timing | Idempotente | Orden | Notas |
|---|---|---|---|---|---|---|---|
| `RENDER_REQUESTED` | UI | Render Engine | `{ documentId, pageIndices: number[], mode: "preview" \| "full", kind: "original" \| "anonymized", scale?: number }` | sync | sí | none | Solicitud de render de páginas visibles. `kind` **requerido** (ADR-056 §1): identifica el panel que pide, y el motor renderiza **solo** ese lado — antes reconstruía los dos por página, lo que con scroll independiente (ADR-054) hacía que scrollear un panel refrescara el otro. Lo determina el panel emisor, nunca el toggle de sincronización de scroll (ADR-056 §2). `scale?` (ADR-037 §1): ausente → `previewScale`/`fullScale` según `mode`; fuera de `(0, MAX_RENDER_SCALE]` → no-op con `warn`. Un request con escala distinta de una pendiente en cola la descarta; uno en vuelo se aborta cooperativamente (supersede por página, ADR-037 §4), y la entrada de supersede se registra **solo para el `kind` pedido** (ADR-056 §4). |
...
| `GROUP_UPDATE_REQUESTED` | UI | Grouping Engine | `{ documentId, groupId, patch: Partial<Pick<EntityGroup, "replacementMode" \| "replacementValue" \| "enabled" \| "canonicalValue">> & { personGender?: PersonGenderChoice } }` | sync | sí | none | Grouping valida y emite `ENTITY_GROUP_UPDATED` + `GROUP_REPLACEMENT_CHANGED`. `personGender` (ADR-069 §4): `"f"`/`"m"` escriben el campo, **`"neutral"` lo borra** (el tercer estado del selector viaja como valor, no como ausencia de clave), y en los tres casos el motor marca la elección como del humano para que ninguna inferencia posterior la pise. Sobre un grupo de `type` distinto de `Person` se ignora con `warn`. |
