...
7. Abrir y cerrar 10 documentos consecutivos → verificar que la memoria regresa al baseline. **Reparto (ADR-048 §3)**: el E2E ejercita el **flujo** (cada `DOCUMENT_CLOSED` deja estado limpio —sin documento activo, sin preview, sin blob URLs vivos— y el ciclo 10 se comporta como el 1); la **medición de bytes contra baseline** es del gate `test:leak` (`tests/leak/`, Hito 11), porque `performance.measureUserAgentSpecificMemory()` exige `crossOriginIsolated` (COOP/COEP), headers que la app de producción no lleva. **Depende de ADR-051 (PR 17.7)**: el supuesto de ADR-048 §3 —que el ciclo open/close ya era ejercitable— era falso. Hasta ese PR no existe ningún control de UI para cerrar un documento que llegó a `Ready` (solo el banner de `Failed` y el cancelar de `PasswordDialog`), así que ni este escenario ni el gate `test:leak` tienen ciclo que medir.
