...
**Cabral legal**: `shared` no puede importar Web Workers, pero sí exportar una factory (`createWorkerEntry(...)`) que cada `entry.ts` invoque. Contrato público nuevo → ADR, más cinco migraciones.
