...
| 12b | Sección "Acerca de" con la **atribución visible** (ADR-070) | `apps/react-client` | 12 | pendiente |

> **Estado de la branch del hito (2026-08-14)**: los PRs 10 a 12 están implementados en `feat/hito10.6-reemplazo-por-genero` y **no mergeados** — la revisión de la branch completa devolvió REJECTED por tres puntos. Dos ya están corregidos (el test de `export-engine` que `Export_Engine.md` §13 exige por nombre y que se perdió al partir el PR 11; los nombres y comentarios de test de `grouping-engine` que seguían describiendo la fusión de dos fuentes que ADR-069 retiró). El tercero —la atribución visible— no era implementable sin decidir dónde vive: lo resuelve **ADR-070** y lo entrega el PR 12b.
...
**Lo que hay que cerrar en el PR 12b** (ADR-070): la **atribución CC-BY-2.5-AR visible en el producto**, como sección "Acerca de" al pie del `SettingsDialog` (`ui/Components.md` §2.6), con enlaces al dataset y a la licencia. Lo que ya está hecho y verificado: `NOTICE` con la fuente única —la atribución de UCI se retiró junto con el dato—, la sección de atribución en `README.md`, y `gender-lexicon.provenance.json` con URL, licencia, fecha de descarga y `sha256` del artefacto. Es obligación de licencia, no cortesía: el PR 12b es el que puede tachar el ítem en ADR-060 §"Validación" y ADR-069 §"Validación", y retirar el párrafo de pendiente del `NOTICE`. Los dos puntos abiertos de ADR-060 quedaron resueltos: el marcado de unisex (`F`/`M`/`A`) y la enmienda de procedencia (ADR-069).
