...
| 11a | `PersonGenderChoice` + `personGender` en `GroupUpdateRequested.patch` (ADR-069 §4) | `shared` | 10 | pendiente |
| 11b | Fuente única, artefacto en el bundle, saneamiento (ADR-069 §1-§3) | `scripts/` + `grouping-engine` | — | parcial (labels y escalera hechos en el PR 11) |
| 11c | Inferencia disparada, elección del humano recordada, tests contra la tabla real (ADR-069 §5-§7) | `grouping-engine` | 11a, 11b | pendiente |
| 12 | `PersonGenderSelect` por grupo + marca en el árbol + **atribución visible** | `apps/react-client` | 11a | pendiente |
...
**Lo que hay que cerrar en el PR 12**: la **atribución CC-BY-2.5-AR visible en el producto** (créditos / "Acerca de", no solo el README), más procedencia y hash del artefacto en el repo — y **retirar la atribución de UCI** al retirarse el dato. Es obligación de licencia, no cortesía. Los dos puntos abiertos de ADR-060 quedaron resueltos: el marcado de unisex (`F`/`M`/`A`) y la enmienda de procedencia (ADR-069).
