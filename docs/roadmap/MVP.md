...
**El problema**: `paintReplacements` derivaba el tamaño de fuente **solo de la altura** del bbox y llamaba `fillText` **sin `maxWidth`**, con el texto centrado. Un token más largo que el dato que reemplaza —`[PERSONA 01]` sobre "Eta"— se derramaba hacia los dos lados, más allá del rectángulo blanco, encima de palabras del original que seguían dibujadas debajo. Afecta a `mask`, `synthetic` y `placeholder`; `redact` es inmune. Es el punto 5 de `Cambios para hacer.txt`.
...
> **Corrido el 2026-08-22, y NO PASA.** Dos de los tres documentos que faltaban se fabricaron para esto (`tests/fixtures/generate.ts`) y el gate produjo ocho hallazgos, cuatro de ellos **fugas de dato en el PDF exportado** — el folio a 270° y el nombre del sello a 90° salen en claro, la carátula `Apellido, Nombre` no se detecta, y OCR sobre el propio exportado recupera "Pérez" y "Juan". Detalle en `roadmap/Post_Hito10.8_Pendientes.md` §23. **El Hito 10.5 no puede cerrarse con esto abierto.**
...
2. **El orden de lectura destruía el nombre** (ADR-067, fila 17). Ordenar por `bbox.y` asc invierte cada run vertical y los intercala entre sí: `Echeverria, Marta de los Mercedes` llegaba a NER como `… Mercedes … los … de … Marta … Echeverria,`. **Supersede ADR-063 §4**, cuyo argumento ("arrastra a `ocr-engine`") dejó de valer: `ocr-engine` nunca puebla `rotation`, así que un orden que se ramifica por ese campo no lo alcanza. La primera redacción de §4 ubicaba el run **intercalado** entre el texto horizontal y eso **partía una línea al medio** —el comparador con tolerancia no es transitivo—, con lo que el bbox de la entidad partida se tragaba el run: una ocurrencia en `x = 250` salía en `x = 10`. Se corrigió emitiendo los runs en una pasada aparte.
...
2. **Una entidad partida en dos líneas tapa las dos líneas enteras** (ADR-074). `mapSpanToWords` calcula **un** bbox como unión de las palabras del match; con las palabras en renglones distintos esa unión es la envolvente. Medido: `Diego Ramos Vargas` produce **557,2 × 18,2 pt**, casi el ancho útil de la página. No hay fuga —tapa de más, nunca de menos— pero destruye texto ajeno, y un documento que borra párrafos deja de servir para lo que se lo anonimiza. `Occurrence`/`OccurrenceRef`/`Replacement` ganan `fragments`: un rectángulo por línea, con `bbox` intacto como envolvente.
...
4. **Un tramo del número de expediente se detecta como teléfono** (ADR-075). `PP-13-00-000000-24/00` produce `[PHONE] "00-000000"`: cada guion es un `\b` válido y los patrones no tienen cómo distinguir un tramo de un número de causa. Falso positivo benigno, pero llena el árbol de entidades que no existen y entrena al usuario a ignorarlo.
...
| 17 | ADR-062 A/B/C: la marca de reemplazo ilegible, de punta a punta — `PreviewUpdated.degraded`, el veredicto guardado en la entrada de cache, `degraded.store`, `DegradedBadge` | `shared` + `render-engine` + `apps/react-client` (tres commits; el primero mezcla `shared` y `render-engine` — **desvío aceptado**, ver debajo) | 15 | **hecho** |
...
> **Desvío de R-1 en el PR 17, aceptado.** ADR-062 §2 y §7 hicieron el campo `degraded` **opcional** justamente para que `shared` pudiera mergear antes que `render-engine` sin romperle la compilación — o sea, se pagó deuda semántica ("ausente ≡ vacío") para comprar un split de tres commits, y después el primero mezcló los dos módulos igual. El árbol final es idéntico al de cuatro commits separados y ningún contenido está mal, así que se corrige la afirmación en vez de reescribir historia ya pusheada. Mismo criterio y mismo precedente que el desvío del commit `9cd4a00` en el Hito 10.7. **Lo que no puede volver a pasar es pagar el costo de diseño de un split y no hacerlo**: si un ADR justifica un campo opcional por R-1, el split es parte de la entrega, no una sugerencia.

**Los nueve ADRs**: `ADR-077` (código de error para crash de worker), `ADR-078` (la edición manual visible en la UI), `ADR-079` (transferencia real por dirección y payload), `ADR-080` (idle-dispose en el pool, no en el manager), `ADR-081` (el patch combinado se rechaza), `ADR-082` (el usuario corrige el tipo de entidad), `ADR-083` (el panel de conflicto elige tipo, no modo), `ADR-084` ("Ver ocurrencias" empuja el valor al buscador), `ADR-085` (memoria de reclasificación por documento). **Y un décimo, ADR-086**, que no cierra una observación ni un defecto de implementación sino un defecto que apareció **verificando en un browser** una de las nueve: el detector de degradación de ADR-058 §7 medía el encogido vertical de la fuente, que en cuerpo de texto no puede ocurrir, y no el aplastado horizontal, que es el que arruina la legibilidad — pasa a la razón de anchos, con el piso de dibujo escalado y el umbral recalibrado a 0,5. Más la **enmienda §11 de ADR-055** (sanciona el patrón `NerDispatchDecodeFailure` antes de que los otros cuatro motores lo copien).
...
- ~~**La marca de reemplazo degradado** (ADR-058 §7 / ADR-062) sigue sin existir en la UI.~~ **CERRADA (2026-08-20, PR 17)** — las tres filas A/B/C de la tabla de ADR-062 del Hito 10.5 están implementadas y el círculo del `EditReplacementDialog` quedó cerrado: el hint de ancho avisa *antes*, la marca avisa *después* con la medición real de Render, en castellano sin jerga (no dice token, placeholder, bbox ni umbral) y con una línea que aclara que **el dato sigue oculto** — la duda que la palabra "degradado" provoca y no contesta.
  ...
  **Pero verificarla en el browser destapó un quinto defecto que ningún gate detectó** — **decidido por ADR-086 el 2026-08-20** (razón de anchos + piso escalado + umbral 0,5); el diagnóstico que sigue es cómo se encontró, y es el más incómodo de los cinco: `PREVIEW_UPDATED.degraded` llegó **vacío** sobre un reemplazo que el panel dibujaba como una mancha ilegible. El criterio de `Contracts.md` §6 compara `finalSizePx` contra `naturalSizePx` y **los dos chocan contra el mismo piso de `REPLACEMENT_MIN_FONT_PX` (8px)**: en una caja de cuerpo de texto (10-14px) `naturalSizePx` nace clavado en el piso, el bucle no puede bajar ni un píxel, y el cociente da **1.00 por construcción** sin importar cuán largo sea el token. El umbral solo es alcanzable con cajas de ~20px o más — texto de título. Lo que arruina la legibilidad es el squeeze horizontal de `fillText(..., maxWidth)`, que el cociente **no observa**. Corolario: la invariancia de escala que afirman ADR-062 §6 y el comentario del kernel **es falsa cerca del piso** (el piso es absoluto, `boxHeight` escala), así que la misma ocurrencia se declara sana a escala 1 y degradada a escala 2; el test de invariancia pasa porque usa cajas grandes, justo el régimen donde sí vale. Las dos tablas de medición están en `roadmap/Post_Hito10.8_Pendientes.md` §16.

  **ADR-086 lo decide** y agrega tres cosas que el diagnóstico no tenía: el producto de las dos compresiones **se simplifica exactamente** a `anchoDisponible / anchoNatural` —el tamaño final se cancela, así que no son dos mediciones sino una—; con el piso escalado la invariancia pasa a ser **exacta**, verificada a seis decimales sobre seis escalas; y `DEGRADED_FONT_RATIO` **baja a 0,5**, porque con 0,6 el criterio nuevo marcaría placeholders normales en cajas apretadas (`[PERSONA 01]` da 0,579) y sobre-marcar erosiona la única señal que el usuario tiene. **Implementado el 2026-08-20** en dos commits (`shared` y `render-engine`, uno por módulo). Tres tests nuevos, cada uno defendiendo una decisión distinta del ADR y los tres falsificados contra la implementación que los violaría: el que reproduce el defecto cae contra el criterio viejo, el de falsos positivos cae si el umbral vuelve a 0,6, y el de invariancia cae con el piso absoluto.

  **Actualización 2026-08-22**: se corrieron dos documentos más (tablas/justificado y sello, fabricados en `tests/fixtures/generate.ts`). La marca de degradación no fue el problema; lo que apareció es peor y está en `roadmap/Post_Hito10.8_Pendientes.md` §23. Falta todavía el escaneado.

  **Gate manual de ADR-086 §4, corrido en browser sobre 1 de los 4 documentos** (el de texto con nombres cortos; faltan el escaneado, el de tablas/justificado y el de sello, que el repo no tiene y `pdf-lib` no fabrica — ver ADR-086 §4, que registra la cobertura real): con los placeholders automáticos no se enciende ninguna marca —sin falsos positivos, que era la única duda que el cálculo no podía resolver—; con un reemplazo de 56 caracteres escrito a mano se enciende, el recuadro naranja aparece sobre el canvas y el diálogo lo explica en castellano llano; y al acortar el texto **la marca se apaga sola**, que es la regla de "reemplazo, no acumulación" de ADR-062 §2 verificada de punta a punta por primera vez.
...
- **Recursos — punto 2 cerrado con alcance medido (2026-09-23):** investigación,
  ...
  ADR-174 y `PDFs_Pesados_Y_Exportacion_Plan.md` cerrados para implementación;
  todavía no tiene banco ni resultados.
...
- ~~Auto-update: `electron-updater` en Windows; Sparkle vendoreado en macOS, con firma EdDSA propia (ADR-131 §3/§4).~~ **CERRADO** — macOS y Windows validan cada actualización con una clave Ed25519 propia. En Windows la firma también ata versión, nombre y SHA-512 del instalador para impedir *replay* (ADR-137). La primera instalación sigue sin Authenticode hasta integrar SignPath.
...
- **EN VALIDACIÓN, y bloquea la publicación hasta probar el secret real en Actions** — Verificación de actualizaciones con **clave propia en Windows**, al modo de Sparkle. Es el **Hito 11.6**, decidido en ADR-137.

  Implementado el 2026-09-05: la privada entra al workflow por el secret `WINDOWS_UPDATE_PRIVATE_KEY`, la pública SPKI queda horneada en la app y el firmador agrega la metadata al `latest.yml` después de construir el `.exe`. El callback de `electron-updater` comprueba firma, versión, nombre y SHA-512 antes de aceptar la descarga; ausencia, formato inválido, archivo alterado o clave desconocida fallan cerrados.

  La privada no se guarda ni se imprime: entra por `stdin`, y el firmador además exige que corresponda a la pública horneada. **Misma raíz de confianza propia que macOS, y el mismo límite**: una cuenta con capacidad de ejecutar Actions con acceso al secret podría firmar. La protección sí cubre a quien altere o reemplace artefactos después del workflow.
