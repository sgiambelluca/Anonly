...
- **Si hubo choque** (ADR-174): `ManualEntityResult.heldConflictIds` no vacío → **no** hay toast de alta; se abre `ManualOverlapDialog` (§6.3). La decisión de "agregado exitoso" es `heldConflictIds.length === 0` **y** un grupo con el valor; `occurrenceCount > 0` solo no alcanza (lo cuenta antes de agrupar).
...
- **Cuándo**: se abre solo, después de un agregado manual (cualquiera de las tres vías) cuyo resultado trae `heldConflictIds`; y desde el aviso de conflicto de una fila cuando el conflicto tiene `heldManual` (en vez de `ConflictDialog`).
...
- **Acción**: `actions.resolveConflict(conflictId, { winner })` → `CONFLICT_RESOLVE_REQUESTED` con `winner: "manual" | "detected"`. Registra una entrada de deshacer (ADR-172). Toast de confirmación con "Deshacer".
- **Cerrar sin elegir**: toast de **advertencia persistente** (no se va solo) — *"Quedó un choque sin resolver en «X». El export está bloqueado hasta que elijas."* con **"Resolver"**, que reabre el diálogo. El conflicto queda visible en la fila y bloquea el export (`03_Data_Model.md` §15).
