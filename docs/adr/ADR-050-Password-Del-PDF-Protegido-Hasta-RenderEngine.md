...
- §6.2 (no viaja en eventos del bus) y §6.3 (grep) se conservan; el grep gana una excepción **acotada y enumerada**: `render.engine.ts`, `worker/kernel.ts` y `worker/entry.ts` de `render-engine`, más `types.ts` de `shared` (`LoadDocumentPayload`) y **`src/types.ts` del façade** (`ImportDocumentInput.password` y la firma de `retryWithPassword` en `IPipelineOrchestrator`). Cualquier aparición fuera de esa lista sigue siendo un hallazgo. *(Errata 2026-08-18: la enumeración original omitía `src/types.ts`, que es el tipo público por donde el password entra al Core y por lo tanto no puede faltar; y el filtro `grep -v` original no descartaba los identificadores PascalCase que solo **nombran** el error —`PdfPasswordRequiredError`, `PdfPasswordRequired`—, así que el primer grep real habría dado hallazgos falsos. Redacción corregida en `08_Security_Model.md` §6.3.)*
