...
`synthesizePerson` elige de un pool único y mezclado de quince nombres de pila —Carlos, María, Juan, Eta, José, Laura…— con un `rng` seedado por `(seed, type, indexInType)` (`shared/src/synthesizer.ts`). Hoy, un grupo cuyo `canonicalValue` es "María Gómez" puede salir del sintetizador como "Carlos Sánchez".
...
Y es información que el usuario nota: el modo `synthetic` existe para que el documento se lea como un documento real (ADR-012), y un texto donde "Carlos Sánchez" queda embarazado o "Eta Torres" es "el imputado" no se lee como un documento real. Es el mismo problema de coherencia referencial que motivó todo el Hito 10.6, con otro modo.
