...
**Compuerta 1 — ¿hay imágenes?** De `page.getOperatorList()` se toman los ops `paintImageXObject`, `paintJpegXObject`, `paintImageMaskXObject` y `paintInlineImageXObject`, y de la CTM vigente en cada uno (simulando `save`/`restore`/`transform`) sale el rectángulo donde esa imagen quedó colocada, en puntos de página. Una página sin ninguno de esos ops **termina acá**, sin rasterizar nada ni cargar Tesseract: es el caso de toda página born-digital, y es lo que hace que el costo del §4 del Contexto sea el único que paga un documento de puro texto.
