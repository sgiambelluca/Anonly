...
2. **Configuración de librerías** apuntando al origen propio:
   - Tesseract.js: `langPath` → `/models/tesseract/` (directorio), `corePath` → `/wasm/tesseract/` (directorio: tesseract.js elige ahí la variante `tesseract-core[-simd]-lstm.wasm.js`), `workerPath` → **`/wasm/tesseract/worker.min.js` (archivo, no directorio)**.
