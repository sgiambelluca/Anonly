...
1. **Mirror versionado**: un script del repo (`scripts/mirror-assets.ts`, a crear en el Hito 3) descarga los assets pinneados (URL + revisión + `sha256` declarados en un `assets.lock.json` versionado), verifica el hash y los deposita en el directorio público del build (`apps/react-client/public/models/`, `public/wasm/`). Los binarios **no** se commitean al repo (son ~110 MB); el script corre en build/deploy.
   ...
   > **Agregado (2026-09-03)**: el script **no borra**, y `public/` está en `.gitignore` — así que un asset que sale del lock **sobrevive** en la máquina que lo había bajado y se sigue sirviendo desde el mismo origen como si siguiera pineado. Eso convierte un pin que falta en un bug que anda en una máquina y en ninguna otra: es lo que pasó entre ADR-090 y ADR-119 con los cores de Tesseract. Desde ahora `pnpm assets:mirror` revisa, **por cada directorio que el lock declara**, si quedó algún archivo que el lock ya no nombra, y **falla** listándolos (un aviso que no frena nada no lo lee nadie). No es recursivo y no mira directorios ajenos al lock: `public/pdfjs/`, que llena `copy-pdfjs-assets.ts` desde `pnpm-lock.yaml`, no es asunto de este script.
