...
> **Errata (2026-08-15, hallazgo en verificación manual post-aprobación del hito)**: la lupa contaba menos apariciones de un nombre que el pipeline de detección completo sobre el mismo documento (20 contra 28, en un caso real). No es la limitación de arriba —las 8 que faltaban eran el mismo literal exacto, no variantes de escritura—: `Page.words` sale de partir el texto por whitespace (ADR-020 §1, `pdf-engine`/`ocr-engine` por igual), así que un nombre pegado a puntuación sin espacio ("Gorrister," "¡Gorrister!") queda en un solo `Word` con la puntuación adentro. `normalizeForComparison` (§2 arriba) no la saca —solo mayúsculas, diacríticos y espacios—, así que `slideWordWindowMatches` nunca matcheaba esas ocurrencias contra un query limpio. El NER no tiene este defecto porque opera sobre el texto corrido, no sobre `Word`. Se corrige con `stripEdgePunctuation`, local a `regex-engine` (no una tercera primitiva de `shared`: es un recorte de tokenización de este matcheo puntual, no una definición de "misma línea" o de normalización que otro consumidor comparta), que recorta la puntuación de **borde** —nunca la interna, `"O'Brien"` no se toca— de cada `Word` y de cada token del `value`/`query` antes de comparar. El recorte del lado del query es necesario y no cosmético: sin él, un resultado cuyo `text` incluye la puntuación pegada (`"Gorrister,"`) no se podría volver a encontrar por su propio texto vía "Agregar como…" (§8) — la búsqueda no sería idempotente sobre sus propios resultados. No cambia la decisión de §2: sigue siendo exacta, y `"J. Pérez"` sigue sin matchear `"José Pérez"`. Ver `Regex_Engine.md` v1.5.0 (§6, §13 caso 25, §14, §15 ítem 10d).
...
- `core/Regex_Engine.md` → v1.1.0: `findLiteral` (§6), casos límite y tests. → v1.3.0 por la errata de §2: de dónde salen las dos primitivas, caso límite 16 y su test. → v1.4.0 por la errata de §8: `searchText` (§6, §7), su costo interactivo (§12), casos 20-24 y el ítem 10c. → v1.5.0 por la segunda errata de §2 (2026-08-15): `stripEdgePunctuation` (§6), caso límite 25 y el ítem 10d.
...
- Grep de control de la errata de §2, sobre `packages/`: `function sharesVerticalBand` y `u0300` —la huella de la normalización, que es lo que hay que grepear porque la copia existente se llama distinto (`normalizeForLexicon`)— aparecen **una sola vez cada uno**, en `shared/src/`. Con los PRs 7 y 8 ya cerrados no queda ninguna copia extra en `render-engine/src/worker/kernel.ts` ni en `grouping-engine/src/gender.ts` — la única copia restante de la normalización es `scripts/build-gender-lexicon.ts` (fuera del inventario original de la errata, ver `MVP.md` §4, no bloqueante).
...
- Grep de control: `pdfjs-dist` no se usa para leer texto ni contenido del documento en `apps/react-client` — el hit-test es contra `Page.words`/`wordsInRect`, nunca contra una capa de texto de pdf.js (§4). El paquete sí aparece en `main.tsx` desde ADR-021 §5, ajeno a este ADR: solo configura `GlobalWorkerOptions.workerSrc` para el worker de `pdf-engine`/`render-engine`, nunca parsea ni extrae texto en el cliente.
