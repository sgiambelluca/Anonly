...
> **Nota (2026-08-09 — `AnnotationKind.Degraded` pasa a pintarse solo en `mode: "preview"`)**: §7 no distinguía modo al decidir la implementación (fue el primer `AnnotationKind` que se pinta sobre el canvas `anonymized`, sin precedente al que mirar), y el kernel terminó pintando el recuadro de aviso también en `mode: "full"` — o sea, dentro del PDF exportado que un tercero recibe. Verificado en un documento real: el recuadro se ve, y no hay ninguna afordancia que lo explique, porque ADR-062 dejó la marca accionable del árbol de entidades fuera de este hito. Un aviso de edición permanente en el archivo final, sin nada que lo acompañe, no es lo que §7 buscaba ("una señal accionable sobre una palanca que ya existía"): esa palanca vive en la UI de edición, no en el documento entregado. Se restringe el pintado a `mode: "preview"`; el veredicto (`tamañoEfectivo/tamañoNatural < DEGRADED_FONT_RATIO`) se sigue calculando igual en los dos modos, invariante a la escala como ya establecía §7 — lo único que cambia es si se pinta. **Pendiente, no resuelto acá**: si el export debería tener alguna forma propia de señalar tokens degradados (¿una fila en la leyenda de ADR-059, condicionada a que haya al menos un grupo degradado? ¿nada, y confiar en que el usuario ya lo vio en preview antes de exportar?) — decisión de producto que queda abierta para cuando la marca del árbol de ADR-062 esté implementada y se sepa qué tan seguido ocurre en la práctica.
## Contexto
...
- Sobre "Eta" (3 caracteres), ni `[PRS-01]` entra.
...
1. PDF de texto con nombres cortos ("Eta", "Luz") en medio de párrafos → el caso que motivó el ADR.
...
| **Solo shrink-to-fit (§1), sin repintado** | Cierra el defecto pero `[PRS-01]` sobre "Eta" queda en 4px: correcto e ilegible. Resuelve la superposición y no el problema que la superposición causaba. |
...
> **Errata (2026-08-20, ADR-086)**: el criterio de degradación de §7 —`finalSizePx / naturalSizePx < DEGRADED_FONT_RATIO`— medía **la compresión que no ocurre en cuerpo de texto**. Los dos términos del cociente chocan contra el mismo piso de `REPLACEMENT_MIN_FONT_PX`, así que en una caja de 10-14 px el cociente da 1,00 por construcción y el aviso era inalcanzable fuera de titulares; lo que arruina la legibilidad es el aplastado horizontal de `fillText(..., maxWidth)`, que el cociente no observaba. Y la invariancia de escala que §7 afirma resultó falsa cerca del piso, porque el piso es absoluto y `boxHeight` escala.
>
> **ADR-086 reemplaza el criterio por la razón de anchos** (`anchoDisponible / anchoNatural`), le saca el piso a la referencia y hace que el piso de dibujo escale, y baja `DEGRADED_FONT_RATIO` a 0,5 porque el número viejo medía otra magnitud. El **principio** de §7 no cambia y es lo que ADR-086 conserva: una razón y no un piso en píxeles, con umbral, porque una señal que aparece siempre no es una señal. Todo el resto de este ADR —shrink-to-fit (§1), repintado de línea (§2-§6), leyenda (§8+)— queda intacto.
