...
Con una comparación por substring pareja, agregar `Eta` como persona taparía cada `Anabella` del expediente. Eso no es un riesgo de fuga —tapa de más, no de menos— pero destruye contenido del documento sin avisar, y el usuario no eligió eso: eligió un valor, no un criterio de coincidencia.
...
`searchText` acepta que el **último** sub-token de la consulta sea un **prefijo** del sub-token del documento. Los demás siguen exigiendo igualdad. Con eso `Eta` encuentra `Anabella`, y una consulta a medio tipear (`Juan Pére`) encuentra mientras se escribe, que es lo que cualquiera espera de una lupa.

`findLiteral` **no** lo acepta: todos sus sub-tokens exigen igualdad. Es lo que impide que agregar `Eta` tape cada `Anabella`.
...
| Prefijo solo en el último sub-token (§2) | `includes` en cualquier sub-token | `Eta` matchearía `Susana` y `Mariana`. El caso que el humano pidió es el prefijo (`Eta` → `Anabella`), y es también el que hace que la búsqueda incremental funcione. |
...
- La lupa puede resaltar de más: `Eta` resalta `Anabella`. Es el precio elegido, y del lado donde solo cuesta un resaltado.
...
- Test unit: `searchText("Eta")` encuentra `Anabella`; `findLiteral` con `"Eta"` sobre el mismo documento **no** emite ninguna ocurrencia sobre esa palabra (§2, la asimetría).
