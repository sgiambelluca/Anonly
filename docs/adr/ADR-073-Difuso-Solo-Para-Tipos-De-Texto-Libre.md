...
- **Unit — no-regresión del texto libre**: `"Pablo Roman"` y `"Pablo Rornan"` (`Person`) siguen cayendo en el mismo grupo, con `aliases` de dos entradas. Mismo test para `Organization` y `Address`.
...
|---|---|
| **Subir el umbral global** (0.95, 0.97) | No resuelve nada y rompe lo que funciona. Con 0.95 el IBAN (0.955) se sigue fusionando y el texto libre deja de agruparse: `"Pablo Roman"` vs `"Pablo Rornan"` da 0.909 y quedarían separados, que es el caso para el que el pase difuso existe. Además cambia un número por otro número sin fundamento: la propiedad que hace falta no es "más estricto", es "no aplicable". |
...
- La no-regresión del texto libre es condición de mergeo, no un extra: si `"Pablo Rornan"` deja de agrupar con `"Pablo Román"`, este ADR rompió lo que vino a proteger.
