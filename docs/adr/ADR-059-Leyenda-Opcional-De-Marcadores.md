...
La leyenda que un usuario va a pedir tarde o temprano es la que dice `[PRS-01] = Eta Gómez`. Es la versión útil para quien tiene que trabajar con el documento anonimizado y el original a la vez.
...
|---|---|
| **Leyenda con el valor original (`[PRS-01] = Eta Gómez`)** | Destruye la razón de ser del producto y hace fallar `no-recuperability` (Contexto §3). No es una opción a ofrecer ni siquiera detrás de una advertencia: el archivo exportado circula fuera del control del usuario. Cerrado por tipo en §3. |
