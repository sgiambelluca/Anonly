...
- **Dónde vive el guard cuando el motor no tiene puerto** (`pdf-engine`, único caso — ADR-055 §10): el decoder se escribe igual **en el motor** y se exporta; lo invoca el consumidor real, que ahí es el façade, sobre un `dispatch<unknown>`. La regla es "quien conoce el contrato escribe el guard", no "el guard vive en el mismo archivo que el `await`".
