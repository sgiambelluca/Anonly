...
# Sequential OCR LSTM pool campaign: automatic 2, then test-only overrides 3/4.
...
case "$PHASE" in all|r2-time|memory-cancel) ;; *) echo "Fase desconocida: $PHASE" >&2; exit 1 ;; esac
...
if ! git diff --quiet -- packages/ apps/; then fail "hay cambios sin commitear en packages/ o apps/; no se puede atribuir el banco al producto vigente."; fi
for profile in P1 P2 R1 R2; do
  case "$profile" in
    P1|P2) ;;
...
git diff --quiet -- packages/ apps/ || fail "el build modificó fuentes de producto."
...
run_one() {
  ...
  if ANONLY_OCR_POOL_RUN="$id" ANONLY_OCR_POOL_OUTPUT_DIR="$RUN_DIR" \
...
# Three interleaved time rounds over both negative controls and both OCR corpora.
...
if [[ "$PHASE" == "all" || "$PHASE" == "r2-time" ]]; then
...
if [[ "$PHASE" == "all" || "$PHASE" == "memory-cancel" ]]; then
...
for profile in P2 R2; do
  for arm in 2 3 4; do run_one cancel "$arm" "$profile" 0; done
done
fi
...
for (const profile of includedProfiles) {
  for (const arm of ["3","4"]) for (let round=0;round<3;round++) {
...
if (phase === "all" || phase === "memory-cancel") for (const profile of ["P2","R2"]) {
  for (const arm of ["3","4"]) for(let round=0;round<3;round++) {
...
if [[ "$PHASE" == "all" || -n "${ANONLY_OCR_POOL_PRIOR_DIR:-}" ]]; then
