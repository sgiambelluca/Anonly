...
import { EngineError, EngineErrorCode, EngineId, isEngineErrorCode } from "@anonly/shared";
...
/**
 * `true` para los dos errores que `Render_Engine.md` §11 declara recuperables
 * por página (`RENDER_PAGE_FAILED`, `RENDER_TIMEOUT`), venga el error del host
 * o **deserializado de un worker**.
 *
 * **Por qué no se discrimina con `instanceof`** (ADR-049 §3,
 * `ai/Code_Standards.md` §7). Los renders corren en un `RenderWorker`, y un
 * error lanzado adentro vuelve al host por `postMessage`, que no transporta
 * prototipos: `EngineError.deserialize()` reconstruye siempre un
 * `DeserializedEngineError` genérico (`Contracts.md` §4). El
 * `instanceof RenderPageFailedError` del host daba `false` para **todo** fallo
 * de render real, con dos consecuencias medidas
 ...
 * discriminar — es `code`, que es lo que lee `isEngineErrorCode`.
 */
export function isRetryablePageError(err: unknown): err is EngineError {
  return (
    isEngineErrorCode(err, EngineErrorCode.RENDER_PAGE_FAILED) ||
    isEngineErrorCode(err, EngineErrorCode.RENDER_TIMEOUT)
  );
}
