...
- Decisiones de diseño no triviales, documentadas también como comentario en `render.engine.ts`:
  - `RENDER_REQUESTED` trae `kind` **requerido** desde ADR-056 §1, pero no `replacements`/`annotations` (Contracts.md §8): el motor renderiza **solo el lado pedido**, reconstruyendo su `RenderPageInput` desde el último recordado para esa página (o valores vacíos si nunca se renderizó). Antes reconstruía los dos lados, lo que con el scroll independiente por panel de ADR-054 hacía que scrollear un panel del visor refrescara el otro. La entrada de supersede (`registerPendingRender`, ADR-037 §4) se registra **solo para el `kind` pedido** (ADR-056 §4): registrar los dos abortaría renders en vuelo legítimos del otro panel.
