...
- Nunca importa otro motor ni React (spec §5). Solo `@anonly/shared`, `@xenova/transformers` y `onnxruntime-web` (ADR-001, ADR-006).
...
- Modelo servido first-party (ADR-018): `env.allowRemoteModels = false`, `env.localModelPath = "/models/ner/"`. Nunca HuggingFace en runtime.
